...
# =====================================================================
# 수동 재스캔(rescan_socket) 릴레이 토글 체류 시간 — *프레임 수가 아니라 시간(초)* 으로 지정.
# =====================================================================
# 로직은 기존(프레임 수집 후 _raw_to_metrics)과 동일하되, ON/OFF 토글 체류를 경과 시간으로
# 직접 제어한다. TV 처럼 PSU hold-up 이 짧은 기기가 토글 중 재부팅되면 RESCAN_OFF_DWELL_S 를
# 줄여 튜닝한다 (가장 중요한 노브). 기본값은 기존 RESCAN_N=12 frame × ~78ms ≈ 0.94s 등가.
RESCAN_FRAME_S = 0.078        # process_raw_mode 1 frame(150 sample @ ~1920Hz) 소요 시간 근사 (시간↔frame 환산용)
RESCAN_ON_DWELL_S = 0.94      # ON 상태 측정 체류 (relay ON 이라 재부팅 무관, 측정 정확도용)
RESCAN_OFF_DWELL_S = 0.078     # OFF 상태 측정 체류 ← TV 등 재부팅 시 이 값을 줄인다 (핵심 노브)
RESCAN_EXCLUDE_S = 0.16       # 앞 settle 구간(≈2 frame) 제외 — relay 전이 직후 과도구간 버림
...
class SocketOrchestrator:
    ...
    def rescan_socket(self, sk):
        ...
        # 토글 체류는 시간(초) 기반 — RESCAN_ON/OFF_DWELL_S (모듈 상단 상수, 직접 튜닝).
        # 앞 RESCAN_EXCLUDE_S 초는 relay 전이 settle 구간이라 평균/window 에서 제외.
        # relay-OFF 체류 ≈ RESCAN_OFF_DWELL_S (기본 0.94s, 기존 RESCAN_N=12 frame 등가).
        RESCAN_EMPTY_DI = OFF_VERIFY_MIN_DELTA_I   # 5.0 ADC 미만 = 전류 없음
        RESCAN_EMPTY_DH1 = 150.0                   # charger H1(~700)의 1/4 미만 = 고조파 부하 없음
        # engine mute 는 전체 토글 구간(on+off 체류)을 넉넉히 덮도록 시간→frame 환산.
        _rescan_dwell_frames = int(
            (RESCAN_ON_DWELL_S + RESCAN_OFF_DWELL_S) / max(RESCAN_FRAME_S, 1e-3)
        )
        ...
        with self.lock:
            ...
            try:
                ...
                #    relay-OFF 체류 = RESCAN_OFF_DWELL_S (TV 재부팅 시 이 값을 줄여 튜닝).
                self.relay.set(sk, False)
                time.sleep(OFF_VERIFY_SETTLE_S)
                raw_off = self._capture_raw_for(
                    RESCAN_OFF_DWELL_S, RESCAN_EXCLUDE_S, f"S{sk}-rescan-off"
                )

                # 3) 복원 (ON) — relay-OFF 체류시간 = RESCAN_OFF_DWELL_S 로 직접 제어.
                #    이 값을 줄이면 TV 등이 꺼졌다 재부팅되는 것을 막을 수 있음. 정확도는
                #    동일 feature 함수라 유지(단 체류가 너무 짧으면 OFF window frame 수↓ → 노이즈↑).
                self.relay.set(sk, True)
                time.sleep(OFF_VERIFY_SETTLE_S)
