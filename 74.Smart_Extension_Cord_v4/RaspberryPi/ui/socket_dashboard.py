...
# Settings 팝업 (Limit 안전 한도 편집 / About 팀 정보). import 실패해도 대시보드는 동작.
try:
    from settings_dialog import SettingsDialog
except Exception:  # noqa: BLE001
    SettingsDialog = None

# 꺼짐 임박 소리 알림 (음성 1회 + 삐빅, Pi HDMI 스피커). import 실패해도 대시보드는 동작.
try:
    from audio_alerts import AudioAlerts, compute_alerts
except Exception:  # noqa: BLE001
    AudioAlerts = None
    compute_alerts = None
...
class SocketDashboard:
    ...
    BORDER_SAFETY_WARN = "#FF1744"   # 안전 한도 80% 경고 — 테두리 빨강 깜빡임
    ...
    STATE_LABEL = {
        "EMPTY": ("OFF", "white"),       # 빈 소켓도 'OFF' 로 표기 (회색 유지)
        "PENDING": ("PENDING", "white"),
        "ASSIGNED": ("ON", "white"),
        "DEVICE_OFF": ("OFF", "white"),
    }

    def __init__(self, engine, orchestrator, timer_manager=None, safety_limits=None):
        self.engine = engine
        self.orchestrator = orchestrator
        self.timer_manager = timer_manager   # 소켓별 예약 타이머 카운트다운 표시(읽기전용)
        self.safety_limits = safety_limits   # 기기별 안전 한도 (Settings → Limit 편집/저장)
        ...
        # row2: 절약 전력량 박스 / row3: 오실로스코프 / row4: info
        # hspace 를 줄여 버튼을 카드에 가깝게 붙인다.
        # top 0.95 → 0.91: 맨 위에 Settings 버튼 전용 띠 확보. 버튼을 소켓 제목과 같은
        # 줄(우상단 모서리)에 두면 'Socket 4' 제목과 겹침 — 제목은 포인트 단위 텍스트라
        # 창 크기에 따라 폭이 변해 x 로 피하는 건 불안정, 줄 자체를 분리해야 안전.
        gs = self.fig.add_gridspec(
            5, N_PHYS_SOCKETS,
            height_ratios=[1.0, 0.66, 0.28, 0.80, 0.80],
        ...
        self.socket_pending_labels = []   # PENDING 30s 카운트다운 (state 밑)
        ...
        for i in range(N_PHYS_SOCKETS):
            ...
            pwr_txt = ax.text(0.5, 0.22, "", ha="center", va="center",
                              ...
                              transform=ax.transAxes)
            # PENDING 30s 카운트다운 (state 'PENDING' 바로 밑)
            pend_txt = ax.text(0.5, 0.30, "", ha="center", va="center",
                               color="#bbdcff", fontsize=14, fontweight="bold",
                               transform=ax.transAxes)
            self.socket_device_labels.append(dev_txt)
            self.socket_state_labels.append(state_txt)
            self.socket_power_labels.append(pwr_txt)
            self.socket_pending_labels.append(pend_txt)
            # 타이머 카운트다운 라벨은 카드가 아니라 Timer 버튼 밑(_build_socket_controls)에 생성.
            self.socket_axes.append(ax)

        # =====================================================
        # 전력 정보 박스 (버튼들과 오실로스코프 사이, 가로로 긴 직사각형)
        #   좌측: 실시간 전력 총합(W)  /  우측: 누적 절약 대기전력
        #   (Pi 한글 폰트 깨짐 방지 위해 영어 라벨 사용)
        # =====================================================
        self.ax_energy = self.fig.add_subplot(gs[2, :])
        self.ax_energy.set_xticks([]); self.ax_energy.set_yticks([])
        self.ax_energy.set_facecolor("#142a14")
        for spine in self.ax_energy.spines.values():
            spine.set_edgecolor("#2e7d32")
            spine.set_linewidth(2)
        # 좌: 실시간 전력 총합
        self.power_text = self.ax_energy.text(
            0.02, 0.5, "Real-time Power   0 W", transform=self.ax_energy.transAxes,
            ha="left", va="center", color="#7fd4ff", fontsize=15, fontweight="bold")
        # 우: 누적 절약 대기전력
        self.energy_text = self.ax_energy.text(
            0.98, 0.5, "Standby Saved   0 W·s ( 0.000 Wh )",
            transform=self.ax_energy.transAxes,
            ha="right", va="center", color="#7CFC7C", fontsize=15, fontweight="bold")

        # =====================================================
        # 중단: oscilloscope (dashboard_ui 와 동일 스타일)
        # =====================================================
        self.ax_scope = self.fig.add_subplot(gs[3, :])
        ...
        # 박스 안에서만 보이도록 작은 폰트 + 좁은 줄간격 + 위에서 시작 (clip_on 으로 밖 잘림)
        self.info_text = self.ax_info.text(
            0.012, 0.96, "",
            transform=self.ax_info.transAxes,
            color="white", fontsize=8, family="monospace",
            linespacing=1.1, verticalalignment="top", clip_on=True,
        )
        ...
        # 우상단 Settings 버튼 → SettingsDialog (Limit 안전 한도 편집 / About 팀 정보).
        # y=0.962~: 소켓 제목 줄(그리드 top 0.91 위 ~0.95) 보다 위의 전용 띠.
        ax_set = self.fig.add_axes([0.875, 0.962, 0.095, 0.032])
        self._settings_button = Button(ax_set, "Settings",
                                       color="#2d3a4a", hovercolor="#2d3a4a")
        self._settings_button.label.set_color("#9ec5ff")
        self._settings_button.label.set_fontsize(10)
        self._settings_button.label.set_fontweight("bold")
        self._settings_button.on_clicked(self._on_settings)

        # 꺼짐 임박 소리 알림 (limit/OFF timer): 음성 1회 + 5초 전 삐빅 (Pi HDMI 스피커)
        self.audio_alerts = AudioAlerts() if AudioAlerts else None
        ...
        # 예약 타이머 설정 팝업 (2단계: ON/OFF timer → ▲▼ picker). Timer 버튼 → show(sk).
        self.timer_dialog = (TimerDialog(self.fig, on_confirm=self._on_timer_confirm)
                             if TimerDialog else None)
        # Settings 팝업 (메뉴 → Limit ▲▼ 편집 → safety_limits.json 저장 / About).
        self.settings_dialog = (SettingsDialog(self.fig,
                                               get_limits=self._get_limits,
                                               on_save=self._on_limits_save)
                                if SettingsDialog else None)
        # 팝업 열 때 숨길 배경 버튼들(hover 시 팝업 위로 튀어오르는 것 방지) — 두 팝업 공유
        bg_axes = (
            [b.ax for b in self._toggle_buttons]
            + [b.ax for b in self._rescan_buttons]
            + [b.ax for b in self._timer_buttons]
            + [self._settings_button.ax]
        )
        if self.timer_dialog is not None:
            self.timer_dialog.bg_axes = bg_axes
        if self.settings_dialog is not None:
            self.settings_dialog.bg_axes = bg_axes
    ...
    def _build_socket_controls(self):
        ...
        H = by1 - by0
        # 세로 배치: 카드에 가깝게(밴드 위쪽) ON/OFF·재스캔 → Timer → 카운트다운.
        # hovercolor=face → hover 시 전체 redraw(렉) 방지.
        row_top_y = by0 + H * 0.72      # ON/OFF·재스캔 (밴드 상단 = 카드와 가까움)
        row_top_h = H * 0.26
        row_timer_y = by0 + H * 0.42    # Timer
        row_timer_h = H * 0.24
        count_y = by0 + H * 0.20        # 카운트다운 텍스트
        for i in range(N_PHYS_SOCKETS):
            sk = i + 1
            ...
            bt = Button(ax_t, "OFF", color=self.TOGGLE_OFF_FACE,
                        hovercolor=self.TOGGLE_OFF_FACE)
            ...
            br = Button(ax_r, _LBL_RESCAN, color="#2d3a4a", hovercolor="#2d3a4a")
            ...
            btm = Button(ax_tm, "Timer", color="#3a2d4a", hovercolor="#3a2d4a")
    ...
    # =========================================================
    # Settings (Limit 안전 한도 / About)
    # =========================================================
    def _on_settings(self, _event):
        """Settings 버튼: 메뉴 팝업 열기 (Limit / About)."""
        if self.settings_dialog is None:
            self._set_status("settings dialog not connected")
            return
        self.settings_dialog.show()

    def _get_limits(self):
        """SettingsDialog Limit 화면용 — 현재 한도 dict (safety_limits.json 값)."""
        if self.safety_limits is None:
            return {}
        try:
            return self.safety_limits.get_all()
        except Exception:  # noqa: BLE001
            return {}

    def _on_limits_save(self, limits):
        """SettingsDialog OK → safety_limits.json 에 저장 (재시작 후에도 유지)."""
        if self.safety_limits is None:
            self._set_status("safety limits not connected")
            return
        try:
            saved = self.safety_limits.set_all(limits)
            self._set_status("Limits saved: " + ", ".join(
                f"{d} {('OFF' if int(s) <= 0 else str(int(s)) + 's')}"
                for d, s in saved.items()))
        except Exception as e:  # noqa: BLE001
            self._set_status(f"limits save error: {e}")

    def _on_timer(self, sk, _event):
        """Timer 버튼: ▲▼ 스테퍼 팝업 열기. (status 는 영어 — Pi 한글 폰트 깨짐 방지)"""
        if self.timer_dialog is None:
            self._set_status("timer dialog not connected")
            return
    ...
    def _on_timer_confirm(self, sk, action, seconds):
        ...
        if self.timer_manager is None:
            self._set_status("timer manager not connected")
            return
        try:
            if seconds <= 0:
                self.timer_manager.cancel_timer(sk)
                self._set_status(f"Socket{sk} timer canceled (00:00:00)")
            else:
                self.timer_manager.set_timer(sk, action, seconds)
                self._set_status(
                    f"Socket{sk} {str(action).upper()} timer set {self._fmt_hms(seconds)}")
        except Exception as e:  # noqa: BLE001
            self._set_status(f"Socket{sk} timer error: {e}")
    ...
    def _refresh_toggle_buttons(self):
        ...
        for i, b in enumerate(self._toggle_buttons):
            ...
            b.color = face                      # 기본색
            b.hovercolor = face                 # hover 도 동일색 → hover redraw(렉) 방지
    ...
    def _on_toggle(self, sk, _event):
        ...
        try:
            ...
            self._set_status(f"Socket{sk} relay {'ON' if on else 'OFF'}")
        except Exception as e:  # noqa: BLE001
            self._set_status(f"Socket{sk} toggle error: {e}")

    def _on_rescan(self, sk, _event):
        """재스캔 버튼: 백그라운드 스레드로 실행 (rescan_socket 은 릴레이 토글로 ~수초 걸려
        main thread 에서 돌리면 UI 가 얼어붙음 — mobile 의 async fetch 와 동일하게 비차단).
        status 는 영어로(Pi 한글 폰트 깨짐 방지) — 한글 결과 msg 대신 res 필드로 요약."""
        self._set_status(f"Socket{sk} re-scanning...")
        ...
        def _work():
            try:
                ...
                if res.get("empty"):
                    dev = res.get("device")
                    msg = (f"Socket{sk} re-scan: no load -> {str(dev).upper()} OFF soon"
                           if dev else f"Socket{sk} re-scan: no load -> cleared")
                elif res.get("device"):
                    chg = " (corrected)" if res.get("changed") else ""
                    msg = f"Socket{sk} re-scan: {str(res['device']).upper()}{chg}"
                else:
                    msg = f"Socket{sk} re-scan: kept (no change)"
            except Exception as e:  # noqa: BLE001
                msg = f"Socket{sk} re-scan error: {e}"
            self._set_status(msg)

        threading.Thread(target=_work, daemon=True, name=f"rescan-S{sk}").start()
    ...
    # Socket card update
    # =========================================================
    def _update_socket_card(self, idx, state, device, pending_remaining=None,
                            safety_warning=False, safety_remaining=None):
        ...
        pend_lbl = self.socket_pending_labels[idx]
        ...
        # 안전 한도 80% 경고: 테두리 빨강 깜빡임 (1Hz — wall-clock 위상이라 프레임률 무관)
        warn = bool(safety_warning) and state == "ASSIGNED"
        if warn and (time.time() % 1.0) < 0.5:
            border = self.BORDER_SAFETY_WARN
        rect.set_facecolor(face)
        rect.set_edgecolor(border)
        rect.set_linewidth(4 if warn else 3)
        ...
        # 카운트다운 라벨 (상호배타 재사용): 안전 경고 'Auto-off Ns'(빨강, ASSIGNED 일 때)
        # 또는 PENDING 30s 카운트다운(파랑). 둘 다 아니면 빈 칸.
        # 경고는 y=0.36 으로 올려 전력 텍스트(y=0.22)와 겹침 방지 (PENDING 은 전력 표시 없음).
        if warn and safety_remaining is not None:
            pend_lbl.set_position((0.5, 0.36))
            pend_lbl.set_color("#FF5252")
            pend_lbl.set_text(f"Auto-off {int(safety_remaining)}s")
        elif state == "PENDING" and pending_remaining is not None:
            pend_lbl.set_position((0.5, 0.30))
            pend_lbl.set_color("#bbdcff")
            pend_lbl.set_text(f"{int(pending_remaining)}s")
        else:
            pend_lbl.set_text("")
    ...
    def _update_energy_label(self):
        """좌: 실시간 전력 총합(W, 정격합)  /  우: 누적 절약 대기전력(W·s + Wh)."""
        try:
            pw = float(self.orchestrator.get_active_power_w())
        except Exception:  # noqa: BLE001
            pw = 0.0
        try:
            ws = float(self.orchestrator.get_energy_saved_ws())
        except Exception:  # noqa: BLE001
            ws = 0.0
        self.power_text.set_text(f"Real-time Power   {pw:,.0f} W")
        self.energy_text.set_text(
            f"Standby Saved   {ws:,.0f} W·s ( {ws / 3600.0:.3f} Wh )")
    ...
    def update_frame(self, _frame):
        ...
        for i in range(N_PHYS_SOCKETS):
            ...
            self._update_socket_card(i, s["state"], s["device"],
                                     s.get("pending_remaining"),
                                     s.get("safety_warning", False),
                                     s.get("safety_remaining"))

        # 2.5) 예약 타이머 카운트다운 (읽기전용 — 설정/취소는 mobile UI 에서)
        self._update_timer_labels()

        # 2.6) 꺼짐 임박 소리 알림: limit(safety_warning=잔여 20%) / OFF timer(잔여 20%)
        #      → 음성 1회, 꺼지기 5초 전부터 삐빅 (음성 끝난 뒤부터). ON timer/PENDING 무음.
        if self.audio_alerts is not None and compute_alerts is not None:
            try:
                timers = (self.timer_manager.get_timers()
                          if self.timer_manager is not None else {})
                self.audio_alerts.update(compute_alerts(sk_status, timers))
            except Exception:  # noqa: BLE001
                pass
        ...
        out += self.socket_timer_labels + self.socket_pending_labels
        out += [self.info_text, self.energy_text, self.power_text]
        return out
