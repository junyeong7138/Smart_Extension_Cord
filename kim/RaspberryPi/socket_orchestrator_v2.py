    ...
    def rescan_socket(self, sk):
        ...
        with self.lock:
            ...
            try:
                ...
                _, _, wf_on = self._settle_and_capture(
                    RESCAN_N, RESCAN_EXCLUDE, f"S{sk}-rescan-on"
                )

                # 2) OFF 상태
                self.relay.set(sk, False)
                time.sleep(OFF_VERIFY_SETTLE_S)
                _, _, wf_off = self._settle_and_capture(
                    RESCAN_N, RESCAN_EXCLUDE, f"S{sk}-rescan-off"
                )
                ...
                self._toggle_average_metrics(
                    SCAN_N_RESTORE, exclude_first=0, label=f"S{sk}-rescan-restore"
                )
