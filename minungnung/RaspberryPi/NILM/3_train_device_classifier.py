...
def main():
    ...
    for dev in y_train.unique():
        ...
        class_ranges[dev] = {
            "P_proxy_p10":  float(sub["P_proxy_mean"].quantile(0.10)),
            "P_proxy_p50":  float(sub["P_proxy_mean"].quantile(0.50)),
            "P_proxy_p90":  float(sub["P_proxy_mean"].quantile(0.90)),
            "Irms_adc_p10":  float(sub["Irms_adc_mean"].quantile(0.10)),
            "Irms_adc_p50":  float(sub["Irms_adc_mean"].quantile(0.50)),
            "Irms_adc_p90":  float(sub["Irms_adc_mean"].quantile(0.90)),
            "H1_60_mag_p10":  float(sub["H1_60_mag_mean"].quantile(0.10)),
            "H1_60_mag_p50":  float(sub["H1_60_mag_mean"].quantile(0.50)),
            "H1_60_mag_p90":  float(sub["H1_60_mag_mean"].quantile(0.90)),
        }
