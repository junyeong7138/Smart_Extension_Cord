...
<script>
  ...
  async function refresh() {
    try {
      ...
      // 실시간 전력 (총합은 W, 보조 지표는 ADC proxy)
      const pw = r.power || {};
      document.getElementById('p-main').textContent =
        (pw.total_w === null || pw.total_w === undefined) ? '--' : fmt(pw.total_w, 0) + ' W';
      ...
      for (const i of SOCKETS) {
        ...
        const showW = info.device && info.state === 'ASSIGNED'
          && info.power_w !== null && info.power_w !== undefined;
        const devText = info.device
          ? `<div class="device">${info.device}${showW ? ' · ' + fmt(info.power_w, 0) + ' W' : ''}</div>`
...
def _read_power(engine):
    ...
    """
    if engine is None:
        return {"irms_adc": None, "h1_mag": None, "p_proxy": None, "total_w": None}
    try:
        ...
        if wf:
            return {
                ...
                "total_w":  float(power_meter.total_power_w(engine)),
            }
...
def _read_sockets(orchestrator, engine=None):
    """orchestrator 의 물리 socket 상태 + device 매핑 (+ 기기별 실시간 추정 W).

    orchestrator 미주입 시 None. engine 주입 시 각 ASSIGNED socket 에 power_w 추가.
    """
    if orchestrator is None:
        return None
    try:
        st = orchestrator.get_socket_status()
    except Exception:  # noqa: BLE001
        return None
    if engine is not None and st:
        try:
            dev_power = power_meter.per_device_power_w(engine)
            for info in st.values():
                dev = info.get("device")
                if dev and info.get("state") == "ASSIGNED":
                    info["power_w"] = round(float(dev_power.get(dev, 0.0)), 0)
                else:
                    info["power_w"] = 0.0
        except Exception as e:  # noqa: BLE001
            logging.debug(f"[MOBILE UI] socket power 계산 실패: {e}")
    return st
...
def create_app(relay, engine=None, orchestrator=None):
    ...
    def status():
        return jsonify(
            ...
            sockets=_read_sockets(orchestrator, engine=engine),
