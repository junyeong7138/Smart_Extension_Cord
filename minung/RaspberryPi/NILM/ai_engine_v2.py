...
# v84 (실측 2026-06-01 01:22): dryer(H1≈17000)+cleaner(≈6000)+tv 동시 active 시 전체
# H1≈27600 의 자연 변동(노이즈)이 ±450~520 까지 커져, 고정 임계 400 을 정상 노이즈가
# 계속 초과 → 가짜 off_suspect 반복 발동 → orchestrator 전수 스캔(릴레이 토글)이
# 무한 반복됐다 (drop=498.9/521.9/448.7/422.0 전부 노이즈인데 trigger, 그 사이 사용자가
# 꽂은 충전기는 이 난리에 묻혀 미검출). 큰 HP 부하일수록 절대 노이즈도 비례해 커지므로,
# off_suspect 임계를 "현재 active 부하(baseline_h1 - idle_baseline_h1) 의 일정 비율" 로
# 동적 상향한다. LP 만 active 면 부하가 작아 floor(400) 가 유지돼 작은 charger/tv OFF 도
# 그대로 검출되고, dryer 류가 켜진 동안에만 임계가 올라가 노이즈(≈520)는 거르고
# charger OFF(≈788)/tv OFF(≈1300) 는 잡는다. 0.025 → 27524×0.025≈688 로
# 노이즈(520)와 charger(788) 사이에 위치. (필요시 이 값만 튜닝)
OFF_SUSPECT_LOAD_FRAC = 0.025
...
# v85 (실측 2026-06-01 03:21:48): microwave(sig_h1≈24425) active 중 사용자가 cleaner
# (dH1≈6354) 를 꽂았는데, _is_mode_update 의 허용폭 sig_h1*0.35=8548 안에 들어가
# "microwave 모드 변경" 으로 흡수돼 cleaner 가 영영 미등록됐다 (그 직전 dryer dH1=17839
# 는 8548 초과라 ACTIVE-LABEL FIX 로 정상 등록됨 — 큰 HP 의 35% tolerance 가 통째로 다른
# 가전(cleaner≈6820) 신호를 삼킬 만큼 커서 생긴 갈림). 실측 microwave 자체 duty 변동은
# ≤2500 (false OFF-SUSPECT drop=2243, cond2 false dH1≤1875) 이므로, 어떤 기기든
# "모드 변경" 으로 인정하는 H1 변화에 절대 상한을 둬 "새 가전급" 변화는 흡수 못 하게 한다.
# (가장 작은 HP 가전 cleaner≈6820 보다 작고 microwave duty 노이즈 ≤2500 보다 충분히 큼)
MODE_UPDATE_DH1_CEILING = 4500.0
...
class AIEngine(DSPEngine):
    ...
    def _update_off_suspect(self, win_feat):
        ...
        # v84: 큰 HP 부하 active 시 노이즈가 비례해 커지므로 임계를 동적 상향.
        # (근거는 OFF_SUSPECT_LOAD_FRAC 정의 주석 참고). LP 만 active 면 active_load 가
        # 작아 floor(OFF_SUSPECT_ABS_H1)=400 유지 → 작은 LP OFF 도 그대로 검출.
        active_load_h1 = max(0.0, self.baseline_h1 - self.idle_baseline_h1)
        off_threshold = max(OFF_SUSPECT_ABS_H1, OFF_SUSPECT_LOAD_FRAC * active_load_h1)

        if drop > off_threshold:
            self._off_suspect_consec += 1
            if self._off_suspect_consec >= OFF_SUSPECT_CONSEC:
                if not self.off_suspect:
                    logging.info(
                        f"[OFF-SUSPECT] H1 drop={drop:.1f} > {off_threshold:.0f} "
                        f"(floor={OFF_SUSPECT_ABS_H1:.0f}, 부하비례={OFF_SUSPECT_LOAD_FRAC * active_load_h1:.0f}, "
                        f"active_load={active_load_h1:.0f}, baseline_h1={self.baseline_h1:.1f}, "
                        f"current_h1={current_h1:.1f}, active={list(self.active_devices)}) "
                        f"→ orchestrator 전수 스캔 위임"
                    )
    ...
    def _register_device_from_stable_window(self, win_feat, reason="pending_on", pending=None):
        ...
        if cached_sig is not None and abs(float(cached_sig.get("delta_h1", 0.0))) > abs(d_h1):
            ...
            # v86: 캐시 sig 는 dominance 로 과소측정된 dH1/dI/dPabs *크기* 만 살린다.
            # method/delta_pf 는 "이번에 어떻게 식별됐는지" 를 담은 현재 값으로 유지해야 한다.
            # (실측 2026-06-01 04:05:25): 소켓1 TV 의 초기 ramp 가 charger 로 등록
            # (method=low_power_h1_over_rf_delta_hp) 됐는데, SIG HISTORY 가 1차 실행에서
            # reclassify+백업된 캐시 sig 로 통째 덮어써 method 가 'reclassify_signature_update'
            # 로 바뀌었다. 그 결과 LP RAMP CONTINUATION 의 old_method 게이트(LP_RAMP_SWAP_
            # OLD_METHODS)가 막혀 charger→tv 재라벨이 안 되고, 빈 소켓3 에 유령 tv 가 생성됐다.
            restored = dict(cached_sig)
            restored["method"] = method
            restored["delta_pf"] = d_pf
            self.device_signatures[device] = restored
        else:
            self.device_signatures[device] = {
                "delta_irms": d_irms,
                "delta_h1": d_h1,
    ...
    def _is_mode_update(self, device, d_irms, d_h1):
        ...
        if device == "dryer" and abs(d_h1) > 1500:
            return True
        # v85: 새 가전급(다른 HP 가전 크기) H1 변화는 모드 변경이 아니라 새 ON event 다.
        # 큰 HP(microwave sig≈24425) 의 sig_h1*0.35 tolerance 가 cleaner(≈6820) 같은
        # 별개 가전을 삼키는 것을 막는다 → device in active_devices and not _is_mode_update
        # 분기로 떨어져 ALT/ACTIVE-LABEL FIX 가 진짜 새 기기를 등록한다.
        if abs(d_h1) >= MODE_UPDATE_DH1_CEILING:
            return False
    ...
    def _identify_device_stable(self, win_feat, pre_feat, pre_irms, pre_h1):
        ...
        if low_power_region:
            ...
            if effective_d_h1 < SIGNAL_MIN_DH1:
                ...
                pf_cand = self._pf_lp_candidate(win_feat)
                if pf_cand is not None and pf_cand != signal_candidate:
                    logging.info(
                        f"[PF OVERRIDE] H1 sig={signal_candidate}(d_h1={effective_d_h1:.0f}) -> "
                        f"PF={float(win_feat.get('PF_proxy_mean', 0.0)):.3f} => '{pf_cand}' (drift fix)"
                    )
                    signal_candidate = pf_cand
    ...
    def _low_power_h1_choice(self, d_h1):
        ...
        if d_h1 < CHARGER_TV_H1_CORRECTION_MAX:
            return "tv"
        return None
    def _pf_lp_candidate(self, win_feat):
        """v83: PF(역률) 로 charger/tv 후보 결정. **진폭 drift 불변** 단서.

        H1 크기(boundary 1100) 는 진폭이라 센서 오프셋/온도/접촉저항/HP duty noise 에
        흔들려 "어제 되던 게 오늘 안 되는" 재현성 문제의 주범이다. PF 는 dimensionless
        (P/S) 라 진폭과 무관하고, charger(~0.34)/tv(~0.60) 분포가 명확히 분리된다
        (학습 데이터: charger PF_p90=0.42 < tv PF_p10=0.49, 갭 존재).

        주의: 합산 PF 는 HP(cleaner/dryer/microwave, PF~0.85)가 dominant active 면
        HP 에 지배되어 LP 를 못 비춘다 → 그 경우 None 반환(기존 H1+RF 경로 유지).
        boundary 는 class_ranges 에서 자동 계산하므로 재학습 시 자동 갱신된다.
        """
        dominant_hp = any(
            abs(float(self.device_signatures.get(d, {}).get("delta_h1", 0.0))) > 5000.0
            for d in self.active_devices if d in HIGH_POWER_DEVICES
        )
        if dominant_hp:
            return None

        pf = float(win_feat.get("PF_proxy_mean", 0.0))
        if pf <= 0.0:
            return None

        ch = self.class_ranges.get("charger", {})
        tv = self.class_ranges.get("tv", {})
        ch_hi = float(ch.get("PF_proxy_p90", 0.42))
        tv_lo = float(tv.get("PF_proxy_p10", 0.49))
        if tv_lo > ch_hi:
            boundary = (ch_hi + tv_lo) / 2.0          # 두 분포 사이 갭의 중앙
        else:
            # 갭이 겹치면(드묾) p50 중앙으로 fallback
            boundary = (float(ch.get("PF_proxy_p50", 0.34))
                        + float(tv.get("PF_proxy_p50", 0.60))) / 2.0
        return "charger" if pf < boundary else "tv"
    ...
    def _reclassify_latest_signature(self, win_feat):
        ...
        if abs(new_d_h1) > abs(float(old.get("delta_h1", 0.0))) * 1.10:
            ...
            if sign_flip or too_big_for_lp:
                ...
                return
            # v86: reclassify 는 sig *크기* 만 갱신한다. method 는 원래 등록 시 식별 경로를
            # 담고 있고 LP RAMP CONTINUATION 의 swap 적격 판정(LP_RAMP_SWAP_OLD_METHODS)에만
            # 쓰이므로, 'reclassify_signature_update' 로 덮어쓰면 charger↔tv ramp 재라벨이
            # 막힌다. delta_pf 도 기존 값을 보존한다 (reclassify 입력에 pf 가 없음).
            self.device_signatures[dev_name] = {
                "delta_irms": new_d_irms,
                "delta_h1": new_d_h1,
                "delta_pabs": new_d_pabs,
                "delta_pf": float(old.get("delta_pf", 0.0)),
                "method": str(old.get("method", "reclassify_signature_update")),
            }
            logging.info(
