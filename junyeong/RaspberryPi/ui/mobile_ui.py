...
엔드포인트:
    ...
    GET  /api/limits             기기별 안전 자동차단 한도(초) 조회
    POST /api/limits             한도 저장 (Settings → Limit, safety_limits.json 반영)
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
    ...
    /* 좌: 실시간 전력 / 우: 절약 대기전력 (2열) */
    .power-row { display: flex; justify-content: space-between; align-items: flex-start; gap: 14px; }
    .power-col { flex: 1; min-width: 0; }
    .power-col.right { text-align: right; }
    .power-saved { font-size: 19px; font-weight: 700; color: #7CFC7C; margin-top: 2px; }
    .power-saved small { font-size: 12px; color: #9fdf9f; font-weight: 600; }
    ...
    /* 헤더 (제목 + 우상단 Settings 버튼) */
    .head-row { display: flex; justify-content: space-between; align-items: flex-start; gap: 10px; }
    .btn.settings {
      background: #333; color: #ccc; min-width: 0; padding: 8px 12px;
      font-size: 13px; font-weight: 600; border: 1px solid #555;
    }
    .btn.settings:active { filter: brightness(1.25); }

    /* 안전 한도 80% 경고: 소켓 테두리 빨간 깜빡임 + 카운트다운 */
    @keyframes blink-red {
      0%, 100% { border-color: #ff1744; box-shadow: 0 0 10px rgba(255,23,68,.55); }
      50%      { border-color: #7a2230; box-shadow: none; }
    }
    .socket.warn { animation: blink-red 1s linear infinite; }
    .state .safew { color: #ff5252; font-weight: 700; letter-spacing: 0; }

    /* Settings 팝업: Limit 편집 / About */
    ...
    .limit-name { font-size: 14px; font-weight: 600; color: #ddd; }
    .limit-input {
      width: 92px; background: #1e1e1e; color: #fff; border: 1px solid #555;
      border-radius: 6px; font-size: 16px; padding: 6px 8px; text-align: right;
    }
    .limit-hint { font-size: 11px; color: #888; margin-top: 2px; }
    .about-box { text-align: center; padding: 6px 0 10px; }
    .about-logo { font-size: 24px; font-weight: 800; color: #fff; }
    .about-tag { font-size: 13px; color: #9fdf9f; margin: 4px 0 14px; }
    .about-team-head { font-size: 12px; color: #888; letter-spacing: 3px; margin-bottom: 6px; }
    .about-name { font-size: 15px; color: #ddd; line-height: 1.65; }
    .about-since { font-size: 12px; color: #777; margin-top: 14px; font-style: italic; }
  </style>
</head>
<body>
  <div class="head-row">
  ...
  <div class="power-card">
    <div class="power-row">
      <div class="power-col">
        ...
        <div class="power-main"><span id="p-main">--</span> W</div>
      </div>
      <div class="power-col right">
        <div class="power-head">절약한 대기전력</div>
        <div class="power-saved" id="p-saved">--</div>
      </div>
    </div>
  </div>
  ...
  <div class="row">
    ...
    <button class="btn" onclick="soundTest()">🔊 소리 테스트</button>
  </div>
  <div class="footer">NILM intelligent multi-tap · sound v2</div>
  ...
  <!-- Settings 팝업: 메뉴 → Limit(자동차단 한도 편집) / About(팀 정보) -->
  <div id="set-pop" class="tpop">
    <div class="tpop-card">
      <div class="tpop-title" id="set-title">Settings</div>
      <!-- 1단계: 메뉴 -->
      <div id="set-menu" class="tpop-screen">
        <button class="tbtn" onclick="openLimits()">🔥 과열 방지 (자동차단 한도)</button>
        <button class="tbtn" onclick="openAbout()">ℹ️ About</button>
        <button class="tbtn ghost" onclick="closeSettings()">닫기</button>
      </div>
      <!-- 2-a: Limit 편집 (초 단위, 0 = 무제한) -->
      <div id="set-limit" class="tpop-screen" style="display:none">
        <div class="limit-rows" id="limit-rows"></div>
        <div class="limit-hint">연속 동작 한도(초) · 0 = 무제한</div>
        <div class="tpop-actions">
          <button class="tbtn ghost" onclick="openSettings()">취소</button>
          <button class="tbtn ok" onclick="saveLimits()">OK</button>
        </div>
      </div>
      <!-- 2-b: About (팀 정보) -->
      <div id="set-about" class="tpop-screen" style="display:none">
        <div class="about-box">
          <div class="about-logo">⚡ Watts Up</div>
          <div class="about-tag">Smart NILM Power Strip</div>
          <div class="about-team-head">Team</div>
          <div class="about-name">Junyeong Park</div>
          <div class="about-name">Haram Kim</div>
          <div class="about-name">Namhyung Kwon</div>
          <div class="about-name">Minwoo Lee</div>
          <div class="about-since">since 2026</div>
        </div>
        <button class="tbtn ghost" onclick="openSettings()">뒤로</button>
      </div>
    </div>
  </div>
...
<script>
  ...
  async function refresh() {
    try {
      ...
      // 좌: 실시간 전력 총합(W, 정격합)  /  우: 누적 절약 대기전력
      const pw = Number(r.power_w) || 0;
      document.getElementById('p-main').textContent =
        pw.toLocaleString(undefined, { maximumFractionDigits: 0 });
      const ws = Number(r.saved_ws) || 0;
      document.getElementById('p-saved').innerHTML =
        ws.toLocaleString(undefined, { maximumFractionDigits: 0 }) +
        ' W·s <small>(' + (ws / 3600).toFixed(3) + ' Wh)</small>';
      ...
      for (const i of SOCKETS) {
        ...
        el.innerHTML = `
          <div class="socket-info">
            ...
            ${devText}
            <div class="state">${stateLabel(info.state, relayOn)}<span class="pend" id="pend-${i}"></span><span class="safew" id="safew-${i}"></span></div>
      ...
      applySafety(r.sockets);
      applyAudioAlerts(r.sockets, r.timers);
    } catch (e) {
  ...
  // 안전 한도 80% 경고: 자동차단 남은 시간 카운트다운 (테두리 깜빡임은 CSS .warn).
  // 서버 safety_remaining → deadline 동기 (pending 과 동일 패턴).
  let safetyDeadlines = {};          // sk -> deadline(ms)
  function applySafety(sockets) {
    sockets = sockets || {};
    const seen = {};
    for (const i of SOCKETS) {
      const info = sockets[i] || {};
      if (info.safety_warning && info.safety_remaining != null) {
        seen[i] = 1;
        safetyDeadlines[i] = Date.now() + info.safety_remaining * 1000;
      }
    }
    for (const k in safetyDeadlines) { if (!seen[k]) delete safetyDeadlines[k]; }
    tickSafety();
  }
  function tickSafety() {
    const now = Date.now();
    for (const i of SOCKETS) {
      const el = document.getElementById('safew-' + i);
      if (!el) continue;
      const dl = safetyDeadlines[i];
      if (dl == null) { el.textContent = ''; continue; }
      const rem = Math.max(0, Math.round((dl - now) / 1000));
      el.textContent = ' · 자동차단 ' + rem + 's';
    }
  }
  setInterval(() => { tickTimers(); tickPending(); tickSafety(); }, 1000);

  // ===================== 꺼짐 임박 소리 알림 (음성 + 삐빅) =====================
  // 규칙 (모니터 audio_alerts.py 와 동일):
  //  - 음성 "OO가 곧 꺼질 예정입니다": 잔여 20% 시점 1회 (limit=safety_warning, OFF timer).
  //  - 삐빅: 꺼지기 5초 전부터 꺼질 때까지 반복, 음성 재생 중엔 대기.
  //  - 대상 limit + OFF timer 만 (PENDING/ON timer 무음).
  // 브라우저 자동재생 정책: 첫 터치(pointerdown)에서 오디오 unlock 필요.
  const DEVICE_KO = { charger: '충전기', tv: '티비', cleaner: '청소기',
                      microwave: '전자레인지', dryer: '드라이기' };
  const VOICE_REMAIN_FRAC = 0.2, BEEP_WINDOW_S = 5, BEEP_PERIOD_MS = 900;
  let audioCtx = null, audioReady = false;
  let voiceSpoken = {};      // sk -> 1 (이번 episode 음성 완료)
  let beepDeadlines2 = {};   // sk -> 꺼짐 deadline(ms) — 삐빅 유지 구간
  ...
  function unlockAudio() {
    ...
    try {
      ...
      if (window.speechSynthesis) {           // iOS: 사용자 제스처 안에서 1회 워밍업
        const u = new SpeechSynthesisUtterance(' ');
        u.volume = 0;
        window.speechSynthesis.speak(u);
      }
      audioReady = true;
      // unlock 확인음: resume 완료 *후* 재생 (suspended 상태에서 쏘면 무음).
      // 이 "삑"이 안 들리면 폰 무음 스위치/미디어 볼륨 문제로 판단 가능.
      const p = (audioCtx.state === 'suspended') ? audioCtx.resume() : Promise.resolve();
      p.then(() => playBeep()).catch(() => {});
      showToast('🔊 소리 알림 활성화 (확인음이 안 들리면 무음모드/볼륨 확인)', true);
    } catch (e) { /* 미지원 브라우저 — 무음으로 동작 */ }
  }
  // pointerdown 미지원 구형 브라우저 대비 touchstart/click 백업 (audioReady 가드로 1회만)
  document.addEventListener('pointerdown', unlockAudio, { passive: true });
  document.addEventListener('touchstart', unlockAudio, { passive: true });
  document.addEventListener('click', unlockAudio, { passive: true });

  let lastSpeakT = 0;   // speechSynthesis.speaking 이 stuck 돼도 삐빅이 영원히 안 막히게

  function speakKo(text) {
    if (!window.speechSynthesis) return false;
    try {
      window.speechSynthesis.cancel();   // Android Chrome 큐 멈춤(stuck) 방지
  ...
  function speakOffWarning(sk, device) {
    const text = device
      ? (DEVICE_KO[device] || device) + '가 곧 꺼질 예정입니다'
      : '소켓 ' + sk + ' 전원이 곧 꺼질 예정입니다';
    if (!speakKo(text)) playBeep();      // 음성 미지원/실패 → 삐빅 대체
  }

  // 🔊 소리 테스트 버튼: unlock + 삐빅 + 음성 + 진단 토스트 (모바일 무음 원인 판별용)
  function soundTest() {
    unlockAudio();
    const p = (audioCtx && audioCtx.state === 'suspended')
      ? audioCtx.resume() : Promise.resolve();
    p.then(() => playBeep()).catch(() => {});
    let vmsg = '음성 미지원';
    if (window.speechSynthesis) {
      const vs = window.speechSynthesis.getVoices();
      const ko = vs.filter(v => (v.lang || '').toLowerCase().indexOf('ko') === 0).length;
      vmsg = '보이스 ' + vs.length + '개(한국어 ' + ko + ')';
      speakKo('소리 테스트입니다');
    }
    const st = audioCtx ? audioCtx.state : '생성실패';
    showToast('🔊 테스트 — AudioContext: ' + st + ' · ' + vmsg, true);
  }

  function playBeep() {
    if (!audioCtx) return;
    const t0 = audioCtx.currentTime;
    [[1480, 0], [1180, 0.16]].forEach(([f, dt]) => {     // "삐빅" 2펄스
      const o = audioCtx.createOscillator(), g = audioCtx.createGain();
      o.type = 'sine'; o.frequency.value = f;
      g.gain.setValueAtTime(0, t0 + dt);
      g.gain.linearRampToValueAtTime(0.5, t0 + dt + 0.01);
      g.gain.setValueAtTime(0.5, t0 + dt + 0.08);
      g.gain.linearRampToValueAtTime(0, t0 + dt + 0.09);
      o.connect(g); g.connect(audioCtx.destination);
      o.start(t0 + dt); o.stop(t0 + dt + 0.1);
    });
  }

  function applyAudioAlerts(sockets, timers) {
    sockets = sockets || {}; timers = timers || {};
    const active = {};
    for (const i of SOCKETS) {
      const s = sockets[i] || {};
      const t = timers[i];
      let voice = false, beep = false, rem = null;
      if (s.safety_warning && s.safety_remaining != null) {            // limit
        voice = true; rem = s.safety_remaining;
        if (s.safety_remaining <= BEEP_WINDOW_S) beep = true;
      }
      if (t && t.action === 'off') {                                   // OFF timer
        const tr = t.remaining, dur = t.duration || 0;
        const tv2 = dur > 0 && tr <= dur * VOICE_REMAIN_FRAC;
        const tb = tr <= BEEP_WINDOW_S;
        if (tv2) voice = true;
        if (tb) beep = true;
        if (tv2 || tb) rem = (rem == null) ? tr : Math.min(rem, tr);
      }
      if (voice || beep) active[i] = 1;
      if (voice && !voiceSpoken[i]) {
        voiceSpoken[i] = 1;
        if (audioReady) speakOffWarning(i, s.device);
        else showToast('🔇 화면을 한 번 탭하면 소리 알림이 켜집니다');  // unlock 안내
      }
      if (beep) beepDeadlines2[i] = Date.now() + Math.max(0, rem) * 1000;
      else delete beepDeadlines2[i];
    }
    for (const k in voiceSpoken) { if (!active[k]) delete voiceSpoken[k]; }
  }

  setInterval(() => {
    const now = Date.now();
    let any = false;
    for (const k in beepDeadlines2) {
      if (beepDeadlines2[k] >= now) any = true;
      else delete beepDeadlines2[k];          // 꺼짐 시각 경과 → 삐빅 종료
    }
    // 음성 재생 중엔 삐빅 대기 — 단 speaking 플래그가 stuck 돼도 8초 후엔 무시
    const speaking = window.speechSynthesis && window.speechSynthesis.speaking
      && (now - lastSpeakT < 8000);
    if (any && audioReady && !speaking) playBeep();
  }, BEEP_PERIOD_MS);

  // ===================== Settings (Limit / About) =====================
  const LIMIT_DEVICES = ['dryer', 'microwave', 'cleaner', 'tv', 'charger'];

  function openSettings() {
    document.getElementById('set-title').textContent = 'Settings';
    document.getElementById('set-menu').style.display = 'flex';
    document.getElementById('set-limit').style.display = 'none';
    document.getElementById('set-about').style.display = 'none';
    document.getElementById('set-pop').classList.add('show');
  }
  function closeSettings() {
    document.getElementById('set-pop').classList.remove('show');
  }
  async function openLimits() {
    let limits = {};
    try {
      const r = await fetch('/api/limits').then(x => x.json());
      limits = r.limits || {};
    } catch (e) { /* 연결 실패 시 0 으로 표시 */ }
    const root = document.getElementById('limit-rows');
    root.innerHTML = '';
    for (const d of LIMIT_DEVICES) {
      const row = document.createElement('div');
      row.className = 'limit-row';
      row.innerHTML = `<span class="limit-name">${d}</span>
        <input class="limit-input" id="lim-${d}" type="number" min="0" max="1800"
               step="5" inputmode="numeric" value="${limits[d] != null ? limits[d] : 0}">`;
      root.appendChild(row);
    }
    document.getElementById('set-title').textContent = '과열 방지 — 자동차단 한도 (초)';
    document.getElementById('set-menu').style.display = 'none';
    document.getElementById('set-limit').style.display = 'flex';
  }
  async function saveLimits() {
    const body = {};
    for (const d of LIMIT_DEVICES) {
      const v = parseInt(document.getElementById('lim-' + d).value, 10);
      body[d] = isNaN(v) ? 0 : v;
    }
    try {
      ...
      showToast(r.ok ? '자동차단 한도 저장 완료 (재시작 후에도 유지)'
                     : ('저장 실패: ' + (r.msg || '')), !!r.ok);
    } catch (e) { showToast('저장 오류: ' + e, false); }
    closeSettings();
  }
  function openAbout() {
    document.getElementById('set-title').textContent = 'About';
    document.getElementById('set-menu').style.display = 'none';
    document.getElementById('set-about').style.display = 'flex';
  }
  ...
  function scheduleNext() {
    // 안정화 중(cooldown>0)·PENDING·안전 경고·삐빅 알람 중이면 0.4s 로 빠르게.
    const fast = lastCd > 0 || Object.keys(pendingDeadlines).length > 0
      || Object.keys(safetyDeadlines).length > 0
      || Object.keys(beepDeadlines2).length > 0;
    setTimeout(async () => {
...
def _read_active_power_w(orchestrator):
    """현재 ON(ASSIGNED) 소켓들의 정격전력 합(W). orchestrator 미주입이면 0."""
    if orchestrator is None:
        return 0.0
    try:
        return float(orchestrator.get_active_power_w())
    except Exception:  # noqa: BLE001
        return 0.0


def _read_saved_ws(orchestrator):
    """누적 절약 대기전력(W·s). orchestrator 미주입이면 0."""
    if orchestrator is None:
        return 0.0
    try:
        return float(orchestrator.get_energy_saved_ws())
    except Exception:  # noqa: BLE001
        return 0.0
...
def create_app(relay, engine=None, orchestrator=None, timer_manager=None,
    ...
    @app.route("/")
    def index():
        # no-store: 폰 브라우저가 옛 페이지를 캐시해 새 기능(소리 등)이 안 도는 것 방지
    ...
    def status():
        return jsonify(
            ...
            power_w=_read_active_power_w(orchestrator),
            saved_ws=_read_saved_ws(orchestrator),
    ...
    def limits():
        """기기별 안전 자동차단 한도(초) 조회/저장 (Settings → Limit).

        POST body: {device: seconds, ...} (부분 갱신 가능, 0 = 무제한).
        저장은 safety_limits.json 에 반영 → 재시작 후에도 유지."""
        if safety_limits is None:
            return jsonify(ok=False, msg="safety limits 미연결", limits={})
        if request.method == "POST":
            data = request.get_json(silent=True) or {}
            try:
                saved = safety_limits.set_all(data)
            except Exception as e:  # noqa: BLE001
                logging.warning(f"[MOBILE UI] limits 저장 실패: {e}")
                return jsonify(ok=False, msg=f"저장 오류: {e}",
                               limits=safety_limits.get_all())
            logging.info(f"[MOBILE UI] safety limits 저장: {saved}")
            return jsonify(ok=True, limits=saved)
        return jsonify(ok=True, limits=safety_limits.get_all())
...
def start_in_background(relay, engine=None, orchestrator=None,
                        timer_manager=None, safety_limits=None,
    ...
    표시. timer_manager 를 넘기면 소켓별 ON/OFF 예약 타이머, safety_limits 를 넘기면
    Settings → Limit 한도 편집 기능 활성화. 전부 옵션이라 미주입 시 기존 동작과 동일.
    """
    app = create_app(relay, engine=engine, orchestrator=orchestrator,
                     timer_manager=timer_manager, safety_limits=safety_limits)
    ip = _get_local_ip()
...
if __name__ == "__main__":
    ...
    # 단독 실행용 경로: relay_controller 는 RaspberryPi/hardware/ (2026-06-12 폴더 정리)
    import os
    import sys
    sys.path.append(os.path.abspath(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "hardware")))
