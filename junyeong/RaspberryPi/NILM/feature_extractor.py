...
API:
    extract_frame_features(ch0, ch1, fs_actual) -> dict
        한 frame(약 150 sample)에서 frame-level feature 10종 추출.

    aggregate_window_features(frame_dicts) -> dict
        N(=WINDOW_SIZE)개의 frame feature를 mean/std로 묶어
        window-level feature 20종(10 mean + 10 std) 생성.

    FRAME_FEATURE_NAMES  : frame feature 컬럼 순서 (10개)
    WINDOW_FEATURE_NAMES : window feature 컬럼 순서 (20개)
                          → RF 모델의 입력 컬럼 순서와 일치해야 함

주의:
    - P_proxy 등은 ADC raw (centered) 곱이라 W가 아니다.
      변수명에 _pscale 의미를 두되, 기존 코드와의 호환을 위해
      컬럼명은 P_proxy / Pabs_mean_proxy / Pstd_proxy 그대로 유지.
"""
