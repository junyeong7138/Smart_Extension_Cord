...
class SocketDashboard:
    ...
    def __init__(self, engine, orchestrator, timer_manager=None):
        ...
        # =====================================================
        # 절약 전력량 박스 (버튼들과 오실로스코프 사이, 가로로 긴 직사각형)
        # =====================================================
        ...
        self.energy_text = self.ax_energy.text(
            0.5, 0.5, "절약 전력량  0 W·s", transform=self.ax_energy.transAxes,
            ha="center", va="center", color="#7CFC7C", fontsize=15, fontweight="bold")
    ...
    def _update_energy_label(self):
        """orchestrator 누적 절약 전력량(W·s) → 박스 텍스트. Wh 도 병기."""
        ws = 0.0
        try:
    ...
    def update_frame(self, _frame):
        ...
        out += [self.info_text, self.energy_text]
        return out
