...
AI_VERSION = "ai_engine_v2_2026_06_01_fullscan_v80_h5_ratio_tiebreak"
...
# v2-H5: charger↔tv 의 H5/H1 ratio 극단 게이트 (SUMMARY 단독측정 charger≈0.73 / tv≈0.089,
# Cohen d=3.27). signal(H1)·RF 가 모순일 때만 보수적 tie-break. 동시 ON 실측은 [H5-RATIO]
# 로그로 검증 후 boundary 조정 (H1 짓눌리면 ratio 상승하므로 tv 쪽은 여유를 둠).
H5_RATIO_CHARGER_MIN = 0.50   # ratio ≥ 이 값 → charger (SMPS 5고조파 풍부)
H5_RATIO_TV_MAX = 0.20        # ratio ≤ 이 값 → tv (고조파 적음)
...
# v2.3: PRE-BASELINE FIX 발동 임계. (pre_h1 - idle) / active sig 합 이 이 값 미만이면
# = active 기기 기여가 거의 사라짐 = ORCH toggle drift 로 보고 pre 보정.
# sig 과대평가(cleaner 등) 에도 견고하도록 idle 제거 비율로 판정.
PRE_BASELINE_FIX_ALIVE_RATIO = 0.50
...
class AIEngine(DSPEngine):
    def __init__(self, spi_core, buffer_size=150):
        ...
        # v2.4: 교란으로 OFF 후보를 못 가린 스캔 직후, orchestrator 가 재스캔을 예약하며
        # 이 flag 를 세운다. baseline 을 동결해 미검출 기기의 drop 이 EMA/cooldown-sync 에
        # 흡수되지 않게 하고, off_suspect 가 곧 재발동해 재스캔이 일어나도록 보존한다.
    ...
    def _register_device_from_stable_window(self, win_feat, reason="pending_on", pending=None):
        ...
        if self.active_devices and self.idle_baseline_h1 > 0:
            ...
            # v2.3: idle 오프셋을 뺀 "active 기기 기여가 얼마나 살아있나" 비율로 판정.
            # 기존 `pre_h1 < expected*0.80` 은 idle 오프셋 + sig 과대평가에 취약했다 —
            # cleaner 처럼 sig_h1(8274) 이 실제 안정 기여(~6420) 보다 크게 잡힌 기기가
            # active 면 정상 baseline(6482) 도 expected(8332) 의 78% 라 80% 미만으로 걸려
            # pre 가 8332 로 잘못 부풀려지고, 그 결과 진짜 charger ON(H1 6482→7111, +629)
            # 이 d_h1=-1221 음수로 보여 PENDING ON → ABORT 로 폐기됐다 (실측 2026-05-31
            # 04:16~17: cleaner active 중 충전기 ON 두 번 다 ABORT).
            # 진짜 toggle drift 는 기기가 잠시 꺼져 기여가 거의 사라진 경우이므로
            # (pre - idle) / sig_합 ≈ 0 이다. 이 비율로만 판정한다:
            #   v43 toggle drift: (190-60)/1378 = 0.09 → 보정 (맞음)
            #   cleaner 정상:     (6482-58)/8274 = 0.78 → 보정 안 함 (맞음)
            sig_alive_ratio = (
                (pre_h1 - self.idle_baseline_h1) / sum_active_sig_h1
                if sum_active_sig_h1 > 0 else 1.0
            )
            if expected_baseline_h1 > 0 and sig_alive_ratio < PRE_BASELINE_FIX_ALIVE_RATIO:
                ...
                logging.info(
                    f"[PRE-BASELINE FIX] sig_alive_ratio={sig_alive_ratio:.2f} < {PRE_BASELINE_FIX_ALIVE_RATIO} "
                    f"(pre_H1={pre_h1:.1f}, idle={self.idle_baseline_h1:.1f}, active sigs={sum_active_sig_h1:.1f}); "
        ...
        if device in self.active_devices and not self._is_mode_update(device, d_irms, d_h1):
            ...
            if device in LOW_POWER_DEVICES and d_h1 >= 1000.0:
                # NOTE(2026-06-01 롤백): range bypass(v102) 를 시도했으나, tv ON 직후 tv 자체의
                # multi-stage ramp 변동(d_h1≥1000)을 charger 로 오인 bypass 등록 → ghost charger
                # → baseline 꼬임 → OFF-SUSPECT → tv 오발 OFF 연쇄 발생(로그 nilm_log_20260601_
                # 003519). tv ramp 와 charger 꽂기는 range(둘 다 charger rs < tv) 로 구별 불가라
                # 롤백. charger 가 HP 다수 중 마지막에 묻히는 건 절차(LP 먼저 켜기)로 회피한다.
            ...
            if (device in HIGH_POWER_DEVICES
                ...
                # v2-fix(v80 이식): mode update 흡수 전, HP 크기 신호면 inactive HP range 후보 확인.
                # microwave active 중 cleaner 꽂으면 RF abs 가 microwave 를 dominant 지목 → mode
                # update 흡수 → cleaner 미등록(PENDING). range_scores 가 inactive HP 를 매치하면 그
                # HP 로 등록한다. 실측 (2026-05-31 23:28:13): cleaner dI=110,dH1=5903 → abs
                # microwave:0.94 지만 range cleaner:0.556 매치인데 흡수됨. (주의: dryer range 가
                # 넓어 cleaner ramp 가 dryer 로 라벨될 수 있음 — 확실하려면 cleaner 를 microwave
                # 보다 먼저 켜기. 그래도 미등록보단 등록이 낫다.)
                if device in HIGH_POWER_DEVICES and (d_irms >= LOW_POWER_DI_MAX or d_h1 >= LOW_POWER_DH1_MAX):
                    hp_alt = self._best_inactive_range_candidate(d_irms, d_h1, d_pabs)
                    if hp_alt is not None:
                        logging.info(
                            f"[ON MODE-UPDATE → NEW HP] {device} active + mode update 이지만 inactive "
                            f"'{hp_alt}' range 매치 (dI={d_irms:.1f}, dH1={d_h1:.1f}) → register {hp_alt}"
                        )
                        device = hp_alt
    ...
    def _identify_device_stable(self, win_feat, pre_feat, pre_irms, pre_h1):
        ...
        d_h1 = float(win_feat.get("H1_60_mag_mean", 0.0)) - pre_h1
        d_pabs = float(win_feat.get("Pabs_mean_proxy_mean", win_feat.get("Pabs_mean_proxy", 0.0))) - float(pre_feat.get("Pabs_mean_proxy_mean", pre_feat.get("Pabs_mean_proxy", 0.0)))
        # v2-H5: charger↔tv 구별의 extensive 단서. charger 는 SMPS 정류기라 H5(5고조파)가
        # 풍부 → d_h5/d_h1 ratio 가 charger≈0.73 vs tv≈0.089 로 8배 차이(SUMMARY Cohen d=3.27).
        # H1 은 HP 노이즈로 charger/tv 가 겹치지만(이번 로그 747/893 역전), H5 는 HP 의 H5
        # 노이즈가 작아 더 강건할 여지가 있다. 단 동시 ON 실측은 미검증 → 측정/로깅 + charger/tv
        # 가 모순일 때 ratio 가 극단인 경우에만 보수적으로 tie-break (Pi 로그로 검증 후 확대).
        d_h5 = float(win_feat.get("H5_300_mag_mean", 0.0)) - float(pre_feat.get("H5_300_mag_mean", 0.0))
        h5_h1_ratio = (d_h5 / d_h1) if d_h1 > 200.0 else None
        ...
        if low_power_region:
            _ratio_str = f"{h5_h1_ratio:.3f}" if h5_h1_ratio is not None else "n/a"
            logging.info(
                f"[H5-RATIO] d_h5={d_h5:.1f} d_h1={d_h1:.1f} H5/H1_ratio={_ratio_str} "
                f"(단독기준 charger≈0.73 / tv≈0.09)"
            )
            ...
            if rf_lp_candidates:
                ...
                if rf_best == signal_candidate:
                    ...
                    # v2-H5: signal(H1)·RF 모순으로 여기 온 charger/tv 케이스에서 H5/H1 ratio 가
                    # 극단이면(charger≥0.50 / tv≤0.20) 그걸로 정정. H1 은 HP 노이즈로 짓눌려
                    # 뒤집힐 수 있으나(이번 tv H1 747→charger 영역 오분류), H5 는 charger SMPS
                    # 지문이라 더 강건할 여지. 보수적 극단 게이트만(중간 ratio 는 기존 signal 유지).
                    if h5_h1_ratio is not None and signal_candidate in ("charger", "tv"):
                        h5_says = None
                        if h5_h1_ratio >= H5_RATIO_CHARGER_MIN:
                            h5_says = "charger"
                        elif h5_h1_ratio <= H5_RATIO_TV_MAX:
                            h5_says = "tv"
                        if h5_says is not None and h5_says != chosen:
                            logging.info(
                                f"[H5 TIEBREAK] signal={signal_candidate} → H5/H1 ratio="
                                f"{h5_h1_ratio:.3f} 극단 → {h5_says} 로 정정 "
                                f"(charger SMPS 고조파 지문, RF={rf_best}:{rf_best_conf:.2f})"
                            )
                            chosen = h5_says
                            conf = max(rf_best_conf, 0.55)
                            method = f"low_power_h5_ratio_{rf_src}"
            else:
    ...
    def _update_baseline(self, win_feat):
        ...
        # v2.4: 교란 재스캔 예약 중에도 동결. off_suspect 가 일시 False (스캔 직후 리셋)
        # 라도, 미검출 기기의 drop 을 baseline 이 흡수하지 않도록 막아 off_suspect 가
        # 곧 재발동하게 한다. (이 freeze 가 없으면 event_cooldown>0 분기의 sync 가
        # drop 을 지워 재스캔이 영영 안 일어남 — 실측 2026-05-31 04:43~44 charger 22초 지연.)
        if self._rescan_armed:
            return
