...
# 이벤트/대기 frame 설정
# v: UX 위해 6→4 단축 (≈0.47s→0.31s). ON/OFF 직후 같은 transient 의 중복 트리거만 막으면
# 되는 디바운스라 4 frame 으로도 충분한 마진(ON ramp 보통 1~2 frame). 분류 정확도와 무관.
EVENT_COOLDOWN_FRAMES = 4
...
# v: UX 위해 30→22 단축(≈2.3s→1.7s). false-OFF 재발화 방지가 목적이라 보수적으로만 줄임
# (baseline EMA 가 그 안에 re-sync 됨). 이건 흔한 경로 아님(엣지).
OFF_VERIFY_CANCEL_COOLDOWN_FRAMES = 22
...
class AIEngine(DSPEngine):
    ...
    def _identify_device_stable(self, win_feat, pre_feat, pre_irms, pre_h1):
        ...
        #
        # v_: 단, dH1 이 압도적으로 크면(LP_SIGN_OVERRIDE_DH1 이상) dI 음수여도 reject 하지
        # 않는다. 실측 (2026-06-01, log 180140 vs 180303): 같은 TV ON 을 두 번 켰는데, idle
        # baseline Irms 가 낮게(73) 잡힌 run 은 dI=+10/dH1=1565 로 정상 통과했지만, 높게(94)
        # 잡힌 run 은 dI=-22.5/dH1=1447.5 로 이 가드에 걸려 false reject 됐다. 차이는 센서
        # idle 오프셋 드리프트(±20 ADC)뿐 — TV 는 dI 부호 경계에 걸친 저전력 기기라 취약.
        # dH1 은 원래 reject 사례(162.8)와 실제 TV(~1450~1565) 사이 마진이 압도적이므로, dH1
        # 이 결정적으로 큰 경우엔 dI 음수를 idle 오프셋 탓으로 보고 LP ON 으로 인정한다.
        LP_SIGN_OVERRIDE_DH1 = 1000.0   # 이 이상의 dH1 이면 dI 부호 모순을 idle 오프셋으로 간주
        if chosen in LOW_POWER_DEVICES and d_irms < 0 and d_h1 > 0:
            if d_h1 >= LP_SIGN_OVERRIDE_DH1:
                logging.info(
                    f"[LP SIGN OVERRIDE] chosen={chosen} dI={d_irms:.1f}(음수)이지만 "
                    f"dH1={d_h1:.1f} ≥ {LP_SIGN_OVERRIDE_DH1:.0f} → idle 오프셋으로 dI 부호 "
                    f"뒤집힘 추정, 실제 LP ON 으로 인정 (reject 면제)"
                )
            else:
                logging.info(
