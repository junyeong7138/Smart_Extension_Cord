...
OFF_CONFIRM_RATIO = 0.30          # ratio_h1 < 0.30 → 부재 = OFF 확정 (양수 쪽 경계)
# v90 (2026-06-12 실측 09:20:53): absent 판정역 비대칭화. relay OFF 에 H1 이 *상승*
# (ratio 음수 = HP duty 교란)했는데 |ratio|<0.30 이 ratio=-0.29 를 absent 로 삼켜
# 살아있는 tv 를 강제 차단했다 (ΔI=+10.4 는 "잔존" 신호였는데 무시). 진짜 부재는
# ratio ≈ 0 (실측 +0.01~+0.07) 이므로 음수 쪽은 -0.15 부터 교란으로 보고 ΔI 보조판정에
# 위임한다 (진짜 부재면 ΔI≈0 → 보조판정도 absent → 결과 동일, 교란이면 잘못된 차단 방지).
OFF_SCAN_NEG_DISTURB_RATIO = 0.15
...
# 기기별 안전 자동차단 (장시간 사용 보호): ASSIGNED(기기 동작 중) 연속 시간이 기기별
# 한도(safety_limits.json, 초)를 넘으면 relay OFF + DEVICE_OFF (대기전력 절약에도 포함).
# 한도의 SAFETY_WARN_FRAC 도달 시 get_socket_status 의 safety_warning/safety_remaining 으로
# UI 경고(소켓 테두리 빨간 깜빡임 + 'Auto-off Ns' 카운트다운)를 노출.
# 한도 0 = 무제한. SafetyLimits 미주입 시 기능 전체 비활성 (기존 동작 보존).
SAFETY_WARN_FRAC = 0.8
...
# =====================================================================
# v89 (2026-06-12) 의심 LP 등록 자동 검증 (auto-verify)
# =====================================================================
# HP(dryer/cleaner) 배경 위에서 LP(charger/tv)를 꽂으면 dH1 이 위상 벡터합으로
# 과소측정되어 charger/tv 가 서로 오분류될 수 있다 (실측 08:50: TV dH1=424 → charger).
# 이 오분류는 *꽂는 순간의 수동 측정* 한계라 분류기 임계로는 못 고치고(v41↔v51
# trade-off), 소켓 위치를 알아낸 *후* 그 소켓만 격리 재측정(rescan 동기-펄스)하면
# 정확히 잡힌다 (실측: dryer 배경 TV 재스캔 ΔH1 +1680~1987 → tv).
# → 신호-RF 모순 fallback(_hp) 으로 등록된 LP 에 한해, 등록 AUTO_VERIFY_DELAY_S 후
#   자동 재스캔 1회로 라벨을 검증/정정한다 (수동 재스캔 버튼을 시스템이 대신 누름).
# 지연 이유: TV 는 켜진 직후 ~10s H1 ramp 구간이라 즉시 재면 또 charger 로 보임.
AUTO_VERIFY_LP_ENABLED = True
AUTO_VERIFY_DELAY_S = 12.0
AUTO_VERIFY_METHODS = frozenset({
    "low_power_h1_over_rf_delta_hp",   # HP 배경 + RF delta 모순 → 신호(H1 boundary) 채택
    "low_power_h1_over_rf_abs_hp",     # HP 배경 + RF abs 모순 → 신호 채택
})
...
class SocketOrchestrator:
    ...
    def __init__(self, relay_controller, ai_engine, lock=None, safety_limits=None):
        self.relay = relay_controller
        self.engine = ai_engine
        self.lock = lock or threading.RLock()
        # 기기별 안전 자동차단 한도 (safety_limits.SafetyLimits). 매 monitor tick 마다
        # get(dev) 로 참조하므로 Settings UI 저장이 재시작 없이 즉시 반영된다.
        # None 이면 안전 자동차단 기능 전체 비활성 (기존 동작/self-test 보존).
        ...
        # 안전 한도용: {sk: (device, ASSIGNED 진입 시각)}. monitor loop 가 lazy 기록/만료
        # (reconcile 방식 — _pending_since 와 동일 패턴). device 가 바뀌면 카운트 리셋.
        self._assigned_since = {}

        # v89 auto-verify 예약: {sk: {"due": 발화 시각, "device": 등록 라벨}}.
        # _assign 이 의심 LP 등록일 때 예약, monitor loop 의 _check_auto_verify 가 발화.
        self._auto_verify = {}
            ...
            if actual_on:
                ...
                if cur == SOCKET_STATE_ASSIGNED:
                    ...
                    try:
                        if dev:
                            self.engine.active_devices.pop(dev, None)
                            # baseline sync. v90: *차단 직전 측정값 − 그 기기 sig* 로 보정.
                            # 기존(차단 직전 절대값 그대로)은 방금 릴레이로 끊은 부하가
                            # baseline 에 그대로 남아, 차단 후 baseline ≫ 실제 신호인
                            # '데드존'을 만들었다 (실측 09:21:12 cleaner 수동 OFF: baseline
                            # H1=6267 vs 실제 ~300 → d_h1 이 분 단위로 음수 → 그동안 ON
                            # 이벤트(양의 d_h1 필요) 검출 불가 → TV 재플러그가 PENDING 만료
                            # 3연속). active 가 비면 _update_off_suspect 도 early-return 이라
                            # EMA 외엔 아무도 못 내린다. sig 차감 + idle floor clamp 로
                            # 차단 직후 baseline ≈ 실제 잔여 부하가 되게 한다.
                            wf = getattr(self.engine, "_latest_win_feat", None)
                            if wf:
                                try:
                                    cut_sig = self.engine.device_signatures.get(dev, {}) or {}
                                    cut_di = max(0.0, float(cut_sig.get("delta_irms", 0.0)))
                                    cut_dh = max(0.0, float(cut_sig.get("delta_h1", 0.0)))
                                    idle_i = float(getattr(self.engine, "idle_baseline_irms", 0.0))
                                    idle_h = float(getattr(self.engine, "idle_baseline_h1", 0.0))
                                    self.engine.baseline_irms = max(
                                        idle_i,
                                        float(wf.get("Irms_adc_mean",
                                                     self.engine.baseline_irms)) - cut_di,
                                    )
                                    self.engine.baseline_h1 = max(
                                        idle_h,
                                        float(wf.get("H1_60_mag_mean",
                                                     self.engine.baseline_h1)) - cut_dh,
                                    )
                                except Exception:  # noqa: BLE001
                                    pass
                            logging.info(
                                f"[ORCH] external OFF: AI engine.active_devices['{dev}'] "
                                f"강제 제거 + baseline sync (sig 차감, "
                                f"Irms={self.engine.baseline_irms:.1f}, "
                    ...
                    # v87: HP OFF 시각 stamp. 기존엔 자동 스캔(_finalize_off)만 찍어서
                    # 수동/타이머/안전차단 OFF 경로는 v70 LP 가짜등록 가드가 작동 안 했다.
                    # 실측 (2026-06-12): dryer 안전차단 직후 baseline drift → 가짜 LP ON
                    # 등록 → toggle-id fallback 이 DEVICE_OFF 소켓 릴레이를 켜 dryer 재가동.
    ...
    # =====================================================================
    def rescan_socket(self, sk, clear_label_if_empty=False):
        ...
        with self.lock:
            ...
            try:
                ...
                if d_i < RESCAN_EMPTY_DI and d_h1 < RESCAN_EMPTY_DH1:
                    ...
                    # v91: clear_label_if_empty (auto-verify 전용) — 검증 대상이 '방금
                    # 의심 등록된 라벨'인데 부하가 없으면 그 등록 자체가 phantom 이었던 것.
                    # 라벨을 보존하면 빈 소켓이 PENDING 만료 후 DEVICE_OFF(phantom) 유령으로
                    # 남는다 (실측 09:39:30: 빈 S4 에 'charger OFF' + 수동 재스캔으로도 안
                    # 지워짐). 삭제 → PENDING 만료 시 EMPTY 로 깨끗하게 복귀.
                    if clear_label_if_empty:
                        keep_dev = None
                    else:
                        # 수동 재스캔: 직전 기기 라벨 *보존* → relay ON(PENDING) 재플러그
                        # 대기, 만료 시 그 라벨의 DEVICE_OFF → 대기전력 절약 누적.
                        # (prev_dev=None 인 빈 소켓이면 None 보존 → 만료 시 EMPTY.)
                        keep_dev = prev_dev
                    self.socket_device[sk] = keep_dev
                    self.socket_device_sig[sk] = None
                    self.socket_state[sk] = SOCKET_STATE_PENDING  # relay ON 유지 → 재플러그 대기
                    if clear_label_if_empty and prev_dev:
                        self.last_event = (
                            f"rescan socket{sk}: no load -> PENDING "
                            f"(phantom '{prev_dev}' cleared)"
                        )
                    elif keep_dev:
                        self.last_event = (
                            f"rescan socket{sk}: no load -> PENDING "
                            f"(remember {keep_dev}, -> DEVICE_OFF after timeout)"
                        )
                    else:
                        self.last_event = f"rescan socket{sk}: no load -> PENDING (empty)"
                    logging.info(f"[ORCH][RESCAN] {self.last_event}")
                    result.update(
                        ok=True, empty=True, device=keep_dev, changed=False,
                        msg=(f"소켓{sk}: 부하 없음 (ΔI={d_i:.0f}, ΔH1={d_h1:.0f}) → 대기"
                             + (f" (phantom '{prev_dev}' 라벨 삭제)"
                                if clear_label_if_empty and prev_dev else
                                (f" (재플러그 없으면 곧 '{keep_dev}' OFF 처리)"
                                 if keep_dev else ""))),
                    )
                    return result
        ...
        while not self._stop.is_set():
            try:
                ...
                if not self._identifying:
                    ...
                    self._check_safety_limits()        # 기기별 안전 한도 (80% 경고/초과 차단)
                    self._check_auto_verify()          # v89: 의심 LP 등록 자동 재검증
            except Exception as e:  # noqa: BLE001
    ...
    def _toggle_identify(self, candidates, dev):
        ...
        """
        # (v56 풀프레임 averaging 파라미터는 v91 에서 동기-펄스 측정으로 대체됨 —
        #  RESCAN_OFF_SAMPLES / RESCAN_OFF_PULSES / RESCAN_ON_RECHARGE_S 를 공유)
        ...
        try:
            for sk in candidates:
                ...
                # v91 (2026-06-12): 측정을 재스캔과 동일한 *동기-펄스* 로 교체.
                # 구버전(풀프레임 수집)은 relay-OFF 연속 체류가 ~600ms 라 TV 재부팅
                # 위험이 있었고, before/off 측정 사이 간격이 길어 HP duty 노이즈가
                # 그대로 diff 에 들어갔다. 동기-펄스는 각 OFF 버스트가 ≈33ms (PSU
                # hold-up 안) 이고 인접 ON/OFF 서브프레임 차감으로 HP 배경이 상쇄되며
                # 펄스 N회 평균으로 노이즈를 √N 억제 — 재스캔에서 정확도가 실측 검증된
                # 그 측정이다 (decision 로직: presence/score/v48/v57/v88 은 불변).
                raw_on_sub, raw_off_sub = self._capture_off_pulses(
                    sk, RESCAN_OFF_SAMPLES, RESCAN_OFF_PULSES, RESCAN_ON_RECHARGE_S,
                    f"S{sk}-toggleid"
                )
                _hscale = (float(getattr(self.engine, "buffer_size", 150))
                           / max(int(RESCAN_OFF_SAMPLES), 1))
                i_before, h_before, _ = self._raw_to_metrics(
                    raw_on_sub, harmonic_scale=_hscale)
                i_off, h_off, _ = self._raw_to_metrics(
                    raw_off_sub, harmonic_scale=_hscale)
                got_new = bool(raw_off_sub)
                ...
                logging.info(
                    ...
                    f"i_on_avg={i_before:.1f}, i_off_avg={i_off:.1f} "
                    f"(sync-pulse x{RESCAN_OFF_PULSES}, {RESCAN_OFF_SAMPLES} samples)"
                )
            ...
            # v88 (2026-06-12 실측 08:50:05): LP 토글식별에서 *모든* 후보가 sign-skip
            # (diff_I 전부 음수 — dryer+cleaner duty 노이즈 ±5 가 LP 전류 신호를 압도)
            # → positive 가 비고, 아래 relax-fallback(max diff_I)이 "덜 음수"라는 이유로
            # *빈* 소켓을 채택했다:
            #   S1(실제 TV): diff_I=-5.0, diff_H1=+6617 / S4(빈): diff_I=-2.7, diff_H1=+48
            #   → 빈 S4 에 phantom 매핑 + 실제 소켓 S1 은 PENDING 만료로 전원 차단(이중 사고).
            # diff_I 가 전원 무정보일 때는 raw diff_H1 dominance 로 결정한다: relay OFF 에
            # H1 이 크게(≥H1_DOM_MIN) 빠졌고 2위의 H1_DOM_RATIO 배 이상이면 그 소켓에
            # 진짜 부하가 있다는 물리 증거다.
            # (v57 의 h_clip 은 score 경로용 — 오분류로 sig_dh 가 작게 등록되면(이번 charger
            #  424) 진짜 신호(6617)까지 노이즈로 clip 하므로 여기선 raw diff_H1 을 본다.
            #  절대 하한 + dominance 조건이 HP duty 노이즈 오채택을 막고, positive 후보가
            #  하나라도 있으면 이 분기는 건너뛰어 기존 v56/v48/v57 경로 그대로 동작.)
            if results and not positive:
                H1_DOM_MIN = 500.0     # 최소 H1 하락 (charger sig~650 수준의 신호 영역)
                H1_DOM_RATIO = 3.0     # 1위가 2위의 3배 이상이어야 채택
                by_h1 = sorted(results.items(), key=lambda kv: -kv[1][3])
                top_sk, top_res = by_h1[0]
                top_h1 = top_res[3]
                second_h1 = by_h1[1][1][3] if len(by_h1) >= 2 else 0.0
                if top_h1 >= H1_DOM_MIN and top_h1 >= H1_DOM_RATIO * max(second_h1, 1.0):
                    logging.info(
                        f"[ORCH][TOGGLE-ID] {dev} H1-dominant fallback 채택: socket{top_sk} "
                        f"diff_H1={top_h1:+.1f} (2위={second_h1:+.1f}; diff_I 전원 음수로 "
                        f"무정보); results={results}"
                    )
                    return top_sk
    ...
    def _assign(self, sk, dev, reason="?"):
        ...
        # v89: 의심 LP 등록이면 자동 검증 예약. method 가 신호-RF 모순 fallback(_hp)일
        # 때만 — 재스캔 결과(..._manual_rescan)나 신뢰 경로(match/high_conf)는 제외라
        # 자기 자신을 다시 예약하는 루프가 생기지 않는다.
        if AUTO_VERIFY_LP_ENABLED and dev in LOW_POWER_DEVICES:
            try:
                method = str(self.engine.device_signatures.get(dev, {}).get("method", ""))
            except Exception:  # noqa: BLE001
                method = ""
            if method in AUTO_VERIFY_METHODS:
                self._auto_verify[sk] = {
                    "due": time.time() + AUTO_VERIFY_DELAY_S, "device": dev,
                }
                logging.info(
                    f"[ORCH][AUTO-VERIFY] socket{sk} {dev} 등록 method={method} "
                    f"(HP 배경 의심 LP) → {AUTO_VERIFY_DELAY_S:.0f}s 후 자동 재스캔 예약"
                )
    ...
    def _scan_one_socket(self, sk):
        ...
        if sig_dh1 > MIN_SIG_H1:
            ...
            # v90: absent 영역은 비대칭 (-0.15 < ratio < +0.30). 음수 쪽으로 큰 ΔH1
            # (relay OFF 에 H1 상승)은 부재가 아니라 측정 교란이다 — 실측 09:20:53:
            # tv@S1 ratio=-0.29 가 구판정(|ratio|<0.30)에서 absent 로 오판돼 강제 차단됨.
            if -OFF_SCAN_NEG_DISTURB_RATIO < ratio_h1 < OFF_CONFIRM_RATIO:
            ...
            # ratio_h1 ≤ -OFF_SCAN_NEG_DISTURB_RATIO: H1 측정 교란 → ΔI 보조 판정
    ...
    # =====================================================================
    def _check_auto_verify(self):
        """예약된 자동 검증 발화: 그 소켓이 *그대로*(같은 라벨로 ASSIGNED) 남아 있으면
        rescan_socket 으로 격리 재측정해 라벨을 검증/정정한다. 그 사이 OFF/수동 재스캔/
        LP swap 으로 상태가 바뀌었으면 예약 폐기 (사람·다른 메커니즘이 먼저 처리한 것)."""
        if not self._auto_verify:
            return
        now = time.time()
        for sk, info in list(self._auto_verify.items()):
            if now < info["due"]:
                continue
            del self._auto_verify[sk]
            if (self.socket_state.get(sk) != SOCKET_STATE_ASSIGNED
                    or self.socket_device.get(sk) != info["device"]):
                logging.info(
                    f"[ORCH][AUTO-VERIFY] socket{sk} 상태 변경됨"
                    f"({self.socket_state.get(sk)}/{self.socket_device.get(sk)}) → 검증 생략"
                )
                continue
            logging.info(
                f"[ORCH][AUTO-VERIFY] socket{sk} ({info['device']}) 자동 재스캔 시작 "
                f"(HP 배경 의심 LP 등록 검증)"
            )
            try:
                # v91: clear_label_if_empty=True — 부하 없으면 의심 등록 자체가 phantom
                # 이었던 것이므로 라벨까지 삭제 (보존하면 빈 소켓에 'OFF' 유령이 남음).
                res = self.rescan_socket(sk, clear_label_if_empty=True) or {}
                logging.info(
                    f"[ORCH][AUTO-VERIFY] socket{sk} 결과: "
                    f"device={res.get('device')}, changed={res.get('changed')}, "
                    f"msg={res.get('msg')}"
                )
            except Exception as e:  # noqa: BLE001
                logging.warning(f"[ORCH][AUTO-VERIFY] socket{sk} 재스캔 실패: {e}")

    # =====================================================================
    # 기기별 안전 자동차단 (safety_limits.json 참조 — 장시간 사용 보호)
    # =====================================================================
    def _safety_limit_for(self, dev):
        """기기 dev 의 한도(초). 0 = 무제한/비활성 (safety 미주입 포함)."""
        if self.safety is None or not dev:
            return 0
        try:
            return int(self.safety.get(dev))
        except Exception:  # noqa: BLE001
            return 0

    def _safety_status(self, sk, now=None):
        """get_socket_status 용 (warning, remaining_s).

        ASSIGNED + 한도>0 일 때만 유효. 경과가 한도의 SAFETY_WARN_FRAC 이상이면
        warning=True (UI 빨간 테두리 깜빡임). monitor 가 아직 진입 시각을 기록하기
        전이면 remaining=full(한도 전체). 그 외 (False, None)."""
        if self.socket_state.get(sk) != SOCKET_STATE_ASSIGNED:
            return False, None
        limit = self._safety_limit_for(self.socket_device.get(sk))
        if limit <= 0:
            return False, None
        ent = self._assigned_since.get(sk)
        if ent is None:
            return False, int(limit)
        elapsed = (now or time.time()) - ent[1]
        remaining = max(0, int(round(limit - elapsed)))
        return (elapsed >= limit * SAFETY_WARN_FRAC), remaining

    def _check_safety_limits(self):
        """ASSIGNED 연속 시간을 lazy 기록하고, 기기별 한도(safety_limits.json) 초과 시
        relay OFF + DEVICE_OFF 로 자동 차단한다 (_check_pending_timeout 과 동일한
        reconcile 방식). 차단은 mobile UI 수동 OFF 와 같은 on_external_relay_change
        경로를 재사용 — 라벨/sig 보존 + engine active 정리 + baseline sync 가 한 번에
        처리되고, DEVICE_OFF 라 대기전력 절약 누적에도 잡힌다."""
        ...
        with self.lock:
            for sk in PHYSICAL_SOCKETS:
                if self.socket_state.get(sk) != SOCKET_STATE_ASSIGNED:
                    self._assigned_since.pop(sk, None)     # ASSIGNED 벗어남 → 카운트 리셋
                    continue
                dev = self.socket_device.get(sk)
                ent = self._assigned_since.get(sk)
                if ent is None or ent[0] != dev:
                    self._assigned_since[sk] = (dev, now)  # 진입/라벨 교체 → 카운트 시작
                    continue
                limit = self._safety_limit_for(dev)
                if limit <= 0:
                    continue                               # 무제한 (tv/charger 기본)
                if now - ent[1] < limit:
                    continue
                # 한도 초과 → 자동 차단.
                # v87: 차단 전 engine mute — 풀부하 HP 를 릴레이로 끊으면 H1/Irms 가
                # 폭락하는 과도구간이 생기는데, mute 없이는 이 과도가 가짜 이벤트
                # (off_suspect 연쇄 / baseline drift 발 가짜 LP ON)로 이어진다
                # (토글식별/OFF스캔이 토글 전 mute 하는 것과 동일한 이유).
                self._mute_engine(ENGINE_MUTE_FRAMES)
                try:
                    self.relay.set(sk, False)
                except Exception as e:  # noqa: BLE001
                    logging.warning(f"[ORCH][SAFETY] relay.set({sk},False) 실패: {e}")
                self._assigned_since.pop(sk, None)
                self.on_external_relay_change(sk)          # ASSIGNED → DEVICE_OFF
                self.last_event = (
                    f"socket{sk}: SAFETY limit {int(limit)}s exceeded ({dev}) "
                    f"-> relay OFF, DEVICE_OFF (standby counted)"
                )
                logging.info(f"[ORCH][SAFETY] {self.last_event}")
    ...
    def get_socket_status(self):
        ...
        safety_warning, safety_remaining}, ...}."""
        with self.lock:
            now = time.time()
            out = {}
            for sk in PHYSICAL_SOCKETS:
                warn, remain = self._safety_status(sk, now)
                out[sk] = {
                    "state": self.socket_state[sk],
                    "device": self.socket_device[sk],
                    "pending_remaining": self._pending_remaining(sk, now),
                    "safety_warning": warn,
                    "safety_remaining": remain,
                }
            return out
...
if __name__ == "__main__":
    ...
    # 케이스 D: 기기별 안전 한도 자동차단 (safety_limits 주입).
    # dryer 한도 1초 → 80% 경고(safety_warning) → 초과 시 relay OFF + DEVICE_OFF.
    print("\n=== 케이스 D: 안전 한도(safety limit) 80% 경고 → 초과 자동차단 ===")

    class MockLimits:
        def get(self, dev):
            return {"dryer": 1}.get(dev, 0)   # dryer 1초 한도, 나머지 무제한

    relay4 = MockRelay(); engine4 = MockEngine(relay4)
    orch4 = SocketOrchestrator(relay_controller=relay4, ai_engine=engine4,
                               safety_limits=MockLimits())
    relay4.set(1, True); orch4.socket_state[1] = SOCKET_STATE_ASSIGNED
    orch4.socket_device[1] = "dryer"; engine4.active_devices["dryer"] = 1
    engine4.device_signatures["dryer"] = {
        "delta_h1": MockEngine.DEV_H1["dryer"], "delta_irms": MockEngine.DEV_I["dryer"],
    }
    orch4._check_safety_limits()                       # 진입 시각 기록 시작
    st = orch4.get_socket_status()[1]
    assert st["safety_remaining"] is not None and not st["safety_warning"], \
        "기록 직후엔 경고가 없어야 함!"
    time.sleep(0.85)                                   # 0.85s ≥ 80% of 1s
    orch4._check_safety_limits()
    st = orch4.get_socket_status()[1]
    assert st["safety_warning"], "80% 도달인데 safety_warning 이 안 뜸!"
    assert orch4.socket_state[1] == SOCKET_STATE_ASSIGNED, "한도 전인데 차단됨!"
    time.sleep(0.25)                                   # 누적 1.10s ≥ 1s 한도
    orch4._check_safety_limits()
    assert orch4.socket_state[1] == SOCKET_STATE_DEVICE_OFF, "한도 초과 자동차단 실패!"
    assert relay4.get_state()[1] is False, "relay 가 OFF 되지 않음!"
    assert "dryer" not in engine4.active_devices, "engine active 정리 안 됨!"
    assert orch4.socket_device[1] == "dryer", "DEVICE_OFF 라벨 보존 실패 (절약 누적용)!"
    print("  ✅ 1s 한도: 80% 경고(safety_warning) → 초과 시 relay OFF + DEVICE_OFF(라벨 보존)")

    # 케이스 E: v88 H1-dominant fallback — 2026-06-12 08:50 실측 phantom 재현.
    # dryer+cleaner 배경에서 TV(S1) 가 charger 로 오분류된 채 토글식별: 모든 후보 diff_I
    # 음수(노이즈) → 구버전은 "덜 음수" 인 빈 S4 를 채택(phantom). diff_H1 증거(S1 +6617
    # vs S4 +48)로 S1 을 채택해야 한다.
    print("\n=== 케이스 E: LP 토글식별 H1-dominant fallback (빈 소켓 phantom 방지) ===")
    relay5 = MockRelay(); engine5 = MockEngine(relay5)
    orch5 = SocketOrchestrator(relay_controller=relay5, ai_engine=engine5)
    for sk in (1, 4):
        relay5.set(sk, True); orch5.socket_state[sk] = SOCKET_STATE_PENDING
    relay5.set(3, True); orch5.socket_state[3] = SOCKET_STATE_ASSIGNED
    orch5.socket_device[3] = "dryer"
    engine5.device_signatures["charger"] = {"delta_irms": 11.6, "delta_h1": 424.1}

    # 토글 측정(동기-펄스)을 실측 로그값으로 스크립트: relay 상태 snapshot → (i, h1).
    def _cap5(sk, n_samples, n_pulses, recharge_s, label=""):
        st_on = dict(relay5.get_state())          # ON 서브프레임 (sk ON)
        st_off = dict(st_on); st_off[sk] = False  # OFF 서브프레임 (sk 만 OFF)
        return [("snap", st_on)], [("snap", st_off)]

    def _met5(raw, harmonic_scale=1.0):
        st = raw[0][1]
        i = 526.0 + (5.0 if not st[1] else 0.0) + (2.7 if not st[4] else 0.0)
        h = 26000.0 - (6617.0 if not st[1] else 0.0) - (48.0 if not st[4] else 0.0)
        return i, h, {}

    orch5._capture_off_pulses = _cap5
    orch5._raw_to_metrics = _met5
    chosen5 = orch5._toggle_identify([1, 4], "charger")
    assert chosen5 == 1, \
        f"FAIL: diff_H1=+6617 인 socket1 이 아니라 socket{chosen5} 채택 (빈 소켓 phantom)!"
    assert relay5.get_state()[1] is True, "FAIL: 토글 후 S1 relay 복원 안 됨!"
    print("  ✅ diff_I 전원 음수 + S1 diff_H1=+6617 → socket1 채택 (빈 S4 phantom 방지)")

    # 케이스 F: v89 auto-verify — 의심 LP 등록(_hp fallback)만 예약되고, 지연 후
    # rescan_socket 이 1회 호출되며, 신뢰 경로/수동재스캔 결과는 예약되지 않아야 한다.
    print("\n=== 케이스 F: 의심 LP 등록 자동 검증 (auto-verify) ===")
    relay6 = MockRelay(); engine6 = MockEngine(relay6)
    orch6 = SocketOrchestrator(relay_controller=relay6, ai_engine=engine6)
    verify_calls = []
    orch6.rescan_socket = lambda sk, clear_label_if_empty=False: (
        verify_calls.append(sk),
        {"ok": True, "device": "tv", "changed": True, "msg": "stub: charger→tv 교정"})[1]

    # F-1: 의심 method → 예약됨
    engine6.device_signatures["charger"] = {
        "delta_irms": 11.6, "delta_h1": 424.1,
        "method": "low_power_h1_over_rf_delta_hp",
    }
    orch6._assign(1, "charger", reason="toggle-id")
    assert 1 in orch6._auto_verify, "의심 LP 등록인데 auto-verify 예약 안 됨!"
    # F-2: due 전엔 발화 안 함
    orch6._check_auto_verify()
    assert verify_calls == [] and 1 in orch6._auto_verify, "지연 전에 발화됨!"
    # F-3: due 경과 + 상태 그대로 → rescan 1회 발화 후 예약 소멸
    orch6._auto_verify[1]["due"] = time.time() - 0.1
    orch6._check_auto_verify()
    assert verify_calls == [1], f"발화 1회가 아님: {verify_calls}"
    assert 1 not in orch6._auto_verify, "발화 후 예약이 안 지워짐!"
    # F-4: 신뢰 경로 method → 예약 안 됨
    engine6.device_signatures["tv"] = {
        "delta_irms": 30.0, "delta_h1": 1500.0,
        "method": "low_power_rf_signal_match_delta",
    }
    orch6._assign(2, "tv", reason="toggle-id")
    assert 2 not in orch6._auto_verify, "신뢰 경로인데 예약됨!"
    # F-5: 재스캔 결과(_manual_rescan) → 예약 안 됨 (자기 재예약 루프 방지)
    engine6.device_signatures["charger"] = {
        "delta_irms": 11.6, "delta_h1": 650.0,
        "method": "low_power_h1_over_rf_delta_hp_manual_rescan",
    }
    orch6._assign(3, "charger", reason="manual_rescan")
    assert 3 not in orch6._auto_verify, "manual_rescan 결과인데 재예약됨 (루프 위험)!"
    # F-6: due 경과 전에 상태가 바뀌면(수동 정정 등) 발화 생략
    engine6.device_signatures["charger"] = {
        "delta_irms": 11.6, "delta_h1": 424.1,
        "method": "low_power_h1_over_rf_delta_hp",
    }
    orch6._assign(4, "charger", reason="toggle-id")
    orch6._auto_verify[4]["due"] = time.time() - 0.1
    orch6.socket_device[4] = "tv"            # 그 사이 라벨이 바뀐 상황
    orch6._check_auto_verify()
    assert verify_calls == [1], "상태 바뀌었는데 발화됨!"
    print("  ✅ 의심 LP 만 예약 → 12s 후 1회 발화, 신뢰경로/재스캔결과/상태변경은 제외")

    # 케이스 G: v90 — 2026-06-12 09:20 실측 "TV 강제차단 + 재인식 불가" 재현.
    # G-1a: OFF-스캔에서 relay OFF 에 H1 이 상승(ratio=-0.29 교란)했는데 absent 로
    #       오판하던 버그 → ΔI 보조판정으로 present 유지.
    # G-1b: 같은 교란 대역이라도 ΔI≈0(진짜 부재)이면 absent 그대로.
    # G-2 : 외부 OFF baseline sync 가 차단되는 기기 sig 를 차감하는지 (데드존 방지).
    print("\n=== 케이스 G: OFF-스캔 음수교란 오판 방지 + 외부 OFF baseline sig 차감 ===")
    relay7 = MockRelay(); engine7 = MockEngine(relay7)
    orch7 = SocketOrchestrator(relay_controller=relay7, ai_engine=engine7)
    relay7.set(1, True)
    orch7.socket_state[1] = SOCKET_STATE_ASSIGNED; orch7.socket_device[1] = "tv"
    engine7.active_devices["tv"] = 1
    engine7.device_signatures["tv"] = {"delta_irms": 31.4, "delta_h1": 1647.7}

    def _tam_disturb(n, exclude_first=0, label=""):
        on = relay7.get_state()[1]
        return (531.0 if on else 520.6,        # diff_i = +10.4 (잔존 전류 신호)
                26000.0 if on else 26484.0,    # diff_h1 = -484 → ratio=-0.29 (교란)
                True)

    orch7._toggle_average_metrics = _tam_disturb
    verdict, _, disturbed = orch7._scan_one_socket(1)
    assert verdict == "present", f"교란(ratio=-0.29)+ΔI=10.4 인데 '{verdict}' (tv 강제차단 재발)!"
    assert relay7.get_state()[1] is True, "스캔 후 relay 복원 안 됨!"

    def _tam_absent(n, exclude_first=0, label=""):
        on = relay7.get_state()[1]
        return (531.0 if on else 530.5,        # diff_i = +0.5 ≈ 0 (부하 없음)
                26000.0 if on else 26400.0,    # diff_h1 = -400 → ratio=-0.24 (교란 대역)
                True)

    orch7._toggle_average_metrics = _tam_absent
    verdict, _, _ = orch7._scan_one_socket(1)
    assert verdict == "absent", f"진짜 부재(ΔI≈0)인데 '{verdict}'!"
    print("  ✅ G-1: 음수교란+ΔI잔존 → present 유지 / 음수교란+ΔI≈0 → absent")

    relay8 = MockRelay(); engine8 = MockEngine(relay8)
    orch8 = SocketOrchestrator(relay_controller=relay8, ai_engine=engine8)
    relay8.set(2, True)
    orch8.socket_state[2] = SOCKET_STATE_ASSIGNED; orch8.socket_device[2] = "cleaner"
    engine8.active_devices["cleaner"] = 2
    engine8.device_signatures["cleaner"] = {"delta_irms": 124.1, "delta_h1": 5906.7}
    engine8._latest_win_feat = {"Irms_adc_mean": 134.4, "H1_60_mag_mean": 6267.2}
    relay8.set(2, False)
    orch8.on_external_relay_change(2)
    assert orch8.socket_state[2] == SOCKET_STATE_DEVICE_OFF
    # 구버전: baseline_h1=6267(차단된 cleaner 포함) → 데드존. 신버전: 6267-5907=360.5
    assert abs(engine8.baseline_h1 - 360.5) < 0.1, \
        f"baseline_h1={engine8.baseline_h1} (sig 차감 안 됨 → 데드존 재발)!"
    assert abs(engine8.baseline_irms - 10.3) < 0.1, f"baseline_irms={engine8.baseline_irms}"
    print(f"  ✅ G-2: 외부 OFF baseline = 차단직전 - sig (H1 {engine8.baseline_h1:.1f}, "
          f"I {engine8.baseline_irms:.1f}) → 재플러그 즉시 인식 가능")

    # 케이스 H: v91 — 2026-06-12 09:39 실측 phantom 유령 재현.
    # auto-verify 의 재스캔이 '부하 없음'을 맞게 판정해도 라벨을 보존해서 빈 소켓이
    # DEVICE_OFF(charger) 유령으로 남던 버그 → clear_label_if_empty=True 면 라벨 삭제,
    # 수동 재스캔(기본값)은 기존대로 라벨 보존 (진짜 기기 재플러그 시나리오 보호).
    print("\n=== 케이스 H: auto-verify phantom 라벨 정리 (빈 소켓 'OFF' 유령 방지) ===")
    relay9 = MockRelay(); engine9 = MockEngine(relay9)
    orch9 = SocketOrchestrator(relay_controller=relay9, ai_engine=engine9)
    # 빈 소켓 측정 시뮬: 모든 캡처가 동일값 → delta 0 → '부하 없음' 판정
    _zero_raw = [("snap", None)]
    orch9._capture_raw_for = lambda dur, ex=0.0, label="": _zero_raw
    orch9._capture_off_pulses = lambda sk, n, p, r, label="": (_zero_raw, _zero_raw)
    orch9._capture_raw = lambda n, ex=0, label="": _zero_raw
    orch9._raw_to_metrics = lambda raw, harmonic_scale=1.0: (
        40.0, 100.0, {"Irms_adc_mean": 40.0, "H1_60_mag_mean": 100.0})

    def _setup_phantom():
        relay9.set(4, True)
        orch9.socket_state[4] = SOCKET_STATE_ASSIGNED
        orch9.socket_device[4] = "charger"
        engine9.active_devices["charger"] = 4
        engine9.device_signatures["charger"] = {"delta_irms": 22.2, "delta_h1": 1063.9}

    _setup_phantom()
    res = orch9.rescan_socket(4, clear_label_if_empty=True)   # auto-verify 경로
    assert res["empty"], "빈 소켓인데 empty 판정 안 됨!"
    assert orch9.socket_device[4] is None, \
        f"phantom 라벨이 안 지워짐: {orch9.socket_device[4]} (유령 'charger OFF' 재발)!"
    assert orch9.socket_state[4] == SOCKET_STATE_PENDING

    _setup_phantom()
    res = orch9.rescan_socket(4)                              # 수동 재스캔 (기본값)
    assert res["empty"] and orch9.socket_device[4] == "charger", \
        "수동 재스캔의 라벨 보존(재플러그 대기)이 깨짐!"
    print("  ✅ auto-verify 빈 소켓 → 라벨 삭제(만료 시 EMPTY) / 수동 재스캔 → 라벨 보존 유지")
