...
# v_속도(2026-06-01): 30→12. event_cooldown 은 stabilizing 오버레이 길이를 정한다(프레임당
# ~0.4s 로 감소). self-소모 측정 경로(off-scan/restore-verify, process_ai_frame 호출)는
# 측정 중 mute 를 스스로 깎으므로 그 프레임수(축소 후 ≤9)보다 충분히 커야 한다 → 12(마진 3).
# 측정이 끝나면 leftover(=12-9≈3≈1.2s)만 남아 stabilizing 이 짧아진다.
ENGINE_MUTE_FRAMES = 12
# _capture_raw 기반 측정(rescan/toggle-id)은 process_ai_frame 을 안 부르므로 측정 중 mute 가
# 안 깎인다 → 큰 값을 주면 그대로 stabilizing 으로 남는다. 이들은 측정 후 settle 만 덮으면
# 되므로 작은 전용 mute 를 쓴다 (stabilizing ≈ 이 값 × ~0.4s).
POST_OP_SETTLE_MUTE = 6

# v2: OFF 전수 스캔 파라미터.
# off_suspect 가 뜨면 ASSIGNED 소켓을 하나씩 relay OFF → ΔH1 측정 → 부재/잔존 판정.
# 릴레이 토글이 물리적 진실이라 signature 추측이 필요 없다.
# v_속도(2026-06-01): 측정 프레임수 축소(OFF 체류 단축). 1 frame=150샘플≈4.7 line cycle 이라
# 3~4 frame 평균이면 RMS/H1 안정. OFF 체류 = SCAN_N_OFF × ~0.1s.
OFF_VERIFY_SETTLE_S = 0.05        # relay 동작 후 측정 전 안정화 대기 (_toggle_average_metrics 도 사용)
SCAN_N_BEFORE = 3                 # 토글 직전 baseline 측정 frame 수
SCAN_N_OFF = 5                    # 토글 OFF 상태 측정 frame 수 (OFF 체류 ≈ 0.5s)
SCAN_EXCLUDE_FIRST = 1            # relay 전이 직후 1 frame 은 settle 중 → 평균 제외
SCAN_N_RESTORE = 1                # 복원 후 안정화 frame 수
...
# OFF-state Irms 가 이 이상이면 HP 배경 부하(dryer 등) 존재로 보고, LP(charger/tv) 끼리의 재식별
# *변경* 을 보류한다. 거대한 HP H1(~21000) 이 작은 LP 고조파(tv~1500/charger~700)를 가려 짧은
# OFF 서브프레임 측정으론 dH1 이 불안정 → charger/tv 오교정 발생(실측 log 184650). (clean LP 단독
# 재스캔은 off I~30 이라 이 임계 아래 → 정상 동작.)
RESCAN_LP_HP_BACKGROUND_IRMS = 150.0
...
# 릴레이 스캔(rescan/toggle-id/off-scan) 진행 중·직후 이 시간(초) 동안은 터치(on_touch)를
# 무시한다. 릴레이 스위칭 EMI 가 터치 GPIO 를 가짜 트리거해 꺼둔 소켓들이 한꺼번에 PENDING
# ON 으로 켜지는 현상 방지 (실측 2026-06-01: 재스캔 연타 중 OFF 소켓 전부 자동 ON).
TOUCH_SCAN_SUPPRESS_COOLDOWN_S = 1.2
...
class SocketOrchestrator:
    ...
    def __init__(self, relay_controller, ai_engine, lock=None):
        ...
        # 릴레이 스캔 종료 시각. on_touch 가 스캔 직후 EMI 가짜 터치를 무시하는 데 사용.
        self._relay_scan_end_t = 0.0
    ...
    # 외부 API: touch press
    # =====================================================================
    def _touch_suppressed(self):
        """지금 터치를 무시해야 하는가? (릴레이 스캔 중이거나 끝난 직후 쿨다운 안)

        릴레이 스캔(_identifying=True) 동안 + 종료 후 TOUCH_SCAN_SUPPRESS_COOLDOWN_S 동안은
        릴레이 스위칭 EMI 가 터치 GPIO 를 가짜로 들뜨게 할 수 있어 터치를 신뢰하지 않는다.
        """
        if self._identifying:
            return True
        return (time.time() - self._relay_scan_end_t) < TOUCH_SCAN_SUPPRESS_COOLDOWN_S
    ...
    def on_touch(self):
        ...
        """
        # 릴레이 스캔(rescan/toggle-id/off-scan) 중이거나 끝난 직후면, 이 터치는 릴레이
        # 스위칭 EMI 가 만든 가짜 트리거일 가능성이 높다 → 무시 (꺼둔 소켓 일괄 ON 방지).
        if self._touch_suppressed():
            self.last_event = "touch ignored - 릴레이 스캔 중/직후 (EMI 가짜 터치 추정)"
    ...
    def rescan_socket(self, sk):
        ...
        # v_안정화(2026-06-01): 65 의 짧은 서브프레임 OFF 펄스 측정은 충전기 같은 작은 LP
        # 부하에서 옆 소켓 TV 의 H1 변동에 오염돼 ΔH1 이 887/1100/203 처럼 크게 흔들렸다
        # (실측 22:03~22:05, 같은 socket2 3회 재스캔 결과 제각각). golden 의 풀프레임 OFF
        # 측정으로 되돌려 측정 안정성을 확보한다. relay-OFF 체류 ≈ 12×78ms ≈ 0.9s 라 TV PSU
        # hold-up 안이라 재부팅도 안 한다(golden 에서 검증된 절충값).
        # WINDOW_SIZE(10 frame) sliding window 를 새 relay 상태로 완전히 교체할 만큼 펌핑하고,
        # 앞 RESCAN_EXCLUDE frame 은 relay 전이 settle 구간이라 평균/window 에서 제외.
        RESCAN_N = 5        # exclude 1 → 4 frame 집계. OFF 체류 ≈ 5×78ms ≈ 0.39s (속도 단축)
        RESCAN_EXCLUDE = 1
        ...
        with self.lock:
            ...
            try:
                ...
                self._mute_engine(POST_OP_SETTLE_MUTE)
                ...
                raw_on = self._capture_raw(RESCAN_N, RESCAN_EXCLUDE, f"S{sk}-rescan-on")

                # 2) OFF 상태 raw 수집 → *즉시 복원* (feature 계산은 복원 후로 미룸).
                #    풀프레임 OFF(~0.9s) 라 동기 서브프레임처럼 옆 소켓 변동에 흔들리지 않고
                #    이 소켓 부하의 평균 ΔI/ΔH1 이 안정적으로 측정된다.
                self.relay.set(sk, False)
                time.sleep(OFF_VERIFY_SETTLE_S)
                raw_off = self._capture_raw(RESCAN_N, RESCAN_EXCLUDE, f"S{sk}-rescan-off")

                # 3) 복원 (ON) — relay-OFF 체류시간 ≈ 0.9s 라 TV 등 PSU hold-up 안이라
                #    재부팅 안 함. 정확도는 학습/추론과 동일 feature 함수라 유지.
                self.relay.set(sk, True)
                time.sleep(OFF_VERIFY_SETTLE_S)
                self._capture_raw(SCAN_N_RESTORE, 0, f"S{sk}-rescan-restore")  # 복원 안정화(버림)

                # 4) feature 계산 (relay 복원된 상태에서 — 학습/추론과 동일 함수)
                _, _, wf_on = self._raw_to_metrics(raw_on)
                _, _, wf_off = self._raw_to_metrics(raw_off)

                if not wf_on or not wf_off:
                ...
                if d_i < RESCAN_EMPTY_DI and d_h1 < RESCAN_EMPTY_DH1:
                    ...
                    self.last_event = f"rescan socket{sk}: 부하 없음 → 비움(PENDING)"
                    logging.info(f"[ORCH][RESCAN] {self.last_event}")
                    result.update(
                        ok=True, empty=True, device=None, changed=(prev_dev is not None),
                        msg=f"소켓{sk}: 연결된 기기 없음 (ΔI={d_i:.0f}, ΔH1={d_h1:.0f}) → 비움",
                    )
                    return result
                ...
                # 5.4) LP 재식별 신뢰 가드: HP 부하(예: dryer)가 켜져 있으면 그 거대한 H1
                # (~21000) 이 작은 LP(tv~1500/charger~700) 의 고조파를 가려, 짧은 OFF 서브프레임
                # 측정으론 dH1 이 불안정해진다. 실측 (2026-06-01 18:50:41, log 184650): dryer
                # ON(off I=407) 중 socket4 의 TV 를 재스캔 → ΔH1 이 1500 이 아니라 241 로 측정돼
                # CHARGER_TV 경계(1100) 아래로 떨어짐 → tv 가 charger 로 오교정. HP 배경이 큰데
                # 기존 LP 라벨을 *다른* LP 로 뒤집는 건 측정 잡음일 가능성이 높으므로 보류하고
                # 기존 라벨을 유지한다 (CLAUDE.md 의 "HP 풀로드 위 LP single-shot 불안정" 물리
                # 한계와 동일 맥락). HP 를 끄고 다시 누르면 정상 재식별된다.
                if (prev_dev in LOW_POWER_DEVICES
                        and device in LOW_POWER_DEVICES
                        and device != prev_dev
                        and i_off > RESCAN_LP_HP_BACKGROUND_IRMS):
                    logging.info(
                        f"[ORCH][RESCAN] socket{sk} 분류={device} 이지만 HP 배경 부하 큼"
                        f"(off I={i_off:.0f} > {RESCAN_LP_HP_BACKGROUND_IRMS:.0f}) → LP 고조파"
                        f" 측정 불안정(ΔH1={d_h1:.0f}), '{prev_dev}'→'{device}' 보류·기존 유지"
                    )
                    result.update(
                        ok=False, device=prev_dev, changed=False,
                        msg=(
                            f"소켓{sk}: 고전력 기기 동작 중이라 '{prev_dev}'↔'{device}' 구분이"
                            f" 불안정 → 변경 보류('{prev_dev}' 유지). 고전력 기기를 끄고 다시"
                            f" 재스캔하세요."
                        ),
                    )
                    return result
                ...
                self._identifying = False
                self._relay_scan_end_t = time.time()  # 스캔 직후 터치 EMI 무시 쿨다운 시작
                self._mute_engine(POST_OP_SETTLE_MUTE)  # rescan 은 raw 측정→짧은 settle mute
    ...
    def _identify_socket_for_device(self, dev):
        ...
        try:
            ...
            VERIFY_N_BEFORE = 3        # v_속도: 4→3 (OFF 체류 단축)
            VERIFY_N_OFF = 5           # v_속도: 6→5 (OFF 체류 ≈ 0.5s)
            VERIFY_EXCLUDE_FIRST = 1
            VERIFY_N_RESTORE = 1       # v_속도: 2→1
    ...
    def _toggle_identify(self, candidates, dev):
        ...
        """
        # v56 averaging 파라미터
        TOGGLE_N_BEFORE = 3         # v_속도: 4→3 (raw 78ms/frame ≈ 0.23s)
        TOGGLE_N_OFF = 5            # v_속도: 6→5. OFF 체류 ≈ 5×78ms ≈ 0.39s
        TOGGLE_EXCLUDE_FIRST = 1    # relay 전이 직후 1 frame 은 settle 중 → 평균에서 제외
        TOGGLE_N_RESTORE = 1        # v_속도: 2→1
        ...
        try:
            for sk in candidates:
                self._mute_engine(POST_OP_SETTLE_MUTE)  # toggle-id 는 raw 측정→짧은 settle mute
