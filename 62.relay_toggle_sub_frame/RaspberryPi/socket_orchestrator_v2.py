...
class SocketOrchestrator:
    ...
    def rescan_socket(self, sk):
        ...
        # OFF 측정: 단일 서브프레임(RESCAN_OFF_SAMPLES 샘플)만 읽어 relay-OFF 체류를 78ms
        #          미만으로 단축 (TV PSU hold-up 회피). OFF 체류 ≈ RESCAN_OFF_SETTLE_S +
        #          RESCAN_OFF_SAMPLES × RESCAN_SAMPLE_S.
        RESCAN_EMPTY_DI = OFF_VERIFY_MIN_DELTA_I   # 5.0 ADC 미만 = 전류 없음
        RESCAN_EMPTY_DH1 = 150.0                   # charger H1(~700)의 1/4 미만 = 고조파 부하 없음
        # engine mute 는 전체 토글 구간(on 체류 + off 서브프레임)을 넉넉히 덮도록 환산.
        _rescan_dwell_frames = int(
            RESCAN_ON_DWELL_S / max(RESCAN_FRAME_S, 1e-3)
            + RESCAN_OFF_SAMPLES * RESCAN_SAMPLE_S / max(RESCAN_FRAME_S, 1e-3)
            + 2
        )
        ...
        with self.lock:
            ...
            try:
                ...
                # 2) OFF 상태 raw 수집 = 단일 서브프레임(RESCAN_OFF_SAMPLES) → *즉시 복원*.
                #    relay-OFF 체류 ≈ RESCAN_OFF_SETTLE_S + RESCAN_OFF_SAMPLES × RESCAN_SAMPLE_S.
                #    이 둘을 줄이면 78ms 미만으로 내려가 TV 재부팅을 피할 수 있음 (핵심 노브).
                self.relay.set(sk, False)
                if RESCAN_OFF_SETTLE_S > 0:
                    time.sleep(RESCAN_OFF_SETTLE_S)
                raw_off = self._capture_subframe(RESCAN_OFF_SAMPLES, f"S{sk}-rescan-off")

                # 3) 복원 (ON) — OFF 체류를 서브프레임 길이로 최소화해 TV 등이 꺼졌다 재부팅되는
                #    것을 막는다. OFF 측정은 단일 서브프레임이라 window std=0 → 기준선 평균값
                #    (Irms/H1/Pabs)만 delta 에 쓰임. 샘플이 적을수록 기준선 노이즈↑ 트레이드오프.
                self.relay.set(sk, True)
                time.sleep(OFF_VERIFY_SETTLE_S)
                self._capture_raw(SCAN_N_RESTORE, 0, f"S{sk}-rescan-restore")  # 복원 안정화(버림)

                # 4) feature 계산 (relay 복원된 상태에서 — 학습/추론과 동일 함수)
                #    wf_off 는 서브프레임(RESCAN_OFF_SAMPLES)이라 고조파를 150 등가로 스케일
                #    보정 → wf_on(풀프레임)과 H1 스케일 일치 (d_h1 왜곡 방지).
                _full = getattr(self.engine, "buffer_size", 150)
                _off_hscale = float(_full) / max(int(RESCAN_OFF_SAMPLES), 1)
                _, _, wf_on = self._raw_to_metrics(raw_on)
                _, _, wf_off = self._raw_to_metrics(raw_off, harmonic_scale=_off_hscale)
