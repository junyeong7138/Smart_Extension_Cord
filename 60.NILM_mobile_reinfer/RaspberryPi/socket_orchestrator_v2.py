...
class SocketOrchestrator:
    ...
    def rescan_socket(self, sk):
        ...
        # WINDOW_SIZE(10 frame) sliding window 를 새 relay 상태로 완전히 교체할 만큼 펌핑.
        # 앞 RESCAN_EXCLUDE frame 은 relay 전이 settle 구간이라 평균/window 에서 제외.
        RESCAN_N = 12       # exclude 2 → 10(=WINDOW_SIZE) frame 집계. relay-OFF 체류 ≈ 12×78ms ≈ 0.9s
        RESCAN_EXCLUDE = 2
        ...
        with self.lock:
            ...
            try:
                ...
                # 3) 복원 (ON) — relay-OFF 체류시간 = OFF raw 수집시간(~78ms/frame)으로만 단축
                #    → TV 등이 꺼졌다 재부팅되지 않음. 정확도는 동일 feature 함수라 유지.
                self.relay.set(sk, True)
                time.sleep(OFF_VERIFY_SETTLE_S)
