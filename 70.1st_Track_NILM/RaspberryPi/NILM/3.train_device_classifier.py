...
def main():
    ...
    # 💡 핵심 해결책: AI가 배경 노이즈(꼼수)를 외우지 못하도록 쓰레기 변수 강제 추방!
    bad_keywords = ['baseline', 'thr', 'delta']
    feature_names = [f for f in feature_names if not any(k in f for k in bad_keywords)]
    
    # 깨끗해진 변수 목록으로 문제지(X)를 다시 인쇄함
    X = X[feature_names]
